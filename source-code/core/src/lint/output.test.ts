...
describe("printReport", async () => {
  ...
  describe("information should include", async () => {
    test("level", async () => {
      ...
      expect(
        (console.error as unknown as MockContext<string[], void>).calls[0][0]
      ).toContain("[error]");
    });
    ...
    test("message", async () => {
      ...
      expect(
        (console.warn as unknown as MockContext<string[], void>).calls[0][0]
    ...
    test("metadata if present", async () => {
      ...
      expect(
        (console.warn as unknown as MockContext<string[], void>).calls[0][1]
...
describe("print", async () => {
  describe("Resource", async () => {
    ...
    test("should print a separator for a resource", async () => {
      ...
      expect(
        (console.info as unknown as MockContext<string[], void>).calls[0][0]
      ).toContain("Resource");
      expect(
        (console.info as unknown as MockContext<string[], void>).calls[0][0]
      ).toContain(lintedResource.languageTag.name);
    });
    ...
    test("should print all reports for the current node", async () => {
      ...
      expect(
        (console.info as unknown as MockContext<string[], void>).calls[1][0]
      ).toContain(`[${lintedResource.lint?.[0].level}]`);
      expect(
        (console.info as unknown as MockContext<string[], void>).calls[1][0]
      ).toContain(`(${lintedResource.lint?.[0].id})`);
      expect(
        (console.info as unknown as MockContext<string[], void>).calls[1][0]
      ).toContain(lintedResource.lint?.[0].message);
    });
  });
  ...
  describe("Message", async () => {
    ...
    test("should print a separator for a message", async () => {
      ...
      expect(
        (console.info as unknown as MockContext<string[], void>).calls[2][0]
      ).toContain("Resource");
      expect(
        (console.info as unknown as MockContext<string[], void>).calls[2][0]
      ).toContain(lintedResource.languageTag.name);
      ...
      expect(
        (console.info as unknown as MockContext<string[], void>).calls[2][0]
      ).toContain(lintedResource.body[0].id.name);
    });
    ...
    test("should print all reports for the current node", async () => {
      ...
      expect(
        (console.info as unknown as MockContext<string[], void>).calls[3][0]
      ).toContain(`[${lintedResource.body[0].lint?.[0].level}]`);
      expect(
        (console.info as unknown as MockContext<string[], void>).calls[3][0]
      ).toContain(`(${lintedResource.body[0].lint?.[0].id})`);
      expect(
        (console.info as unknown as MockContext<string[], void>).calls[3][0]
      ).toContain(lintedResource.body[0].lint?.[0].message);
    });
  });
  ...
  describe("Pattern", async () => {
    ...
    test("should print all reports for the current node", async () => {
      ...
      expect(
        (console.info as unknown as MockContext<string[], void>).calls[4][0]
      ).toContain(`[${lintedResource.body[0].pattern.lint?.[0].level}]`);
      expect(
        (console.info as unknown as MockContext<string[], void>).calls[4][0]
      ).toContain(`(${lintedResource.body[0].pattern.lint?.[0].id})`);
      expect(
        (console.info as unknown as MockContext<string[], void>).calls[4][0]
      ).toContain(lintedResource.body[0].pattern.lint?.[0].message);
    });
  });
});
