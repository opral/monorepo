...
describe("print", async () => {
	describe("Resource", async () => {
		...
		test("should print a separator for a resource", async () => {
			...
			expect((console.info as unknown as MockContext<string[], void>).calls[0][0])
				.toContain('Resource')
			expect((console.info as unknown as MockContext<string[], void>).calls[0][0])
		...
		test("should print all reports for the current node", async () => {
			...
			expect((console.info as unknown as MockContext<string[], void>).calls[1][0])
	...
	describe("Message", async () => {
		...
		test("should print a separator for a message", async () => {
			...
			expect((console.info as unknown as MockContext<string[], void>).calls[2][0])
				.toContain('Resource')
			expect((console.info as unknown as MockContext<string[], void>).calls[2][0])
				.toContain(lintedResource.languageTag.name)
			expect((console.info as unknown as MockContext<string[], void>).calls[2][0])
		...
		test("should print all reports for the current node", async () => {
			...
			expect((console.info as unknown as MockContext<string[], void>).calls[3][0])
	...
	describe("Pattern", async () => {
		test("should not proceed if pattern has no lint reports", async () => {
			print({
				type: 'Resource',
				languageTag: { name: 'en' } as LanguageTag,
				body: [{ type: 'Message', id: {}, lint: [{}], pattern: { type: 'Pattern' } }] as LintedMessage[]
			} as LintedResource)
		...
		test("should print all reports for the current node", async () => {
			...
			expect((console.info as unknown as MockContext<string[], void>).calls[4][0])
