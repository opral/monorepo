	...
	return (
		<Show when={inlangConfig()?.languages}>
			<sl-select
				...
				prop:defaultValue={
					// set to navigator language if available in config
					// otherwise set to all languages
					inlangConfig()?.languages?.includes(navigator.language)
						? [
								inlangConfig()?.languages.filter((l) => l !== inlangConfig()?.referenceLanguage),
								navigator.language,
						  ]
						: inlangConfig()?.languages
				}
			...
			>
				...
				<sl-divider class="mt-2 mb-0 border border-surface-3" />
