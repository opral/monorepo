...
type Node<Name extends NodeName, Extension extends ExtensionInformation =
	Record<string, unknown>> = {
		type: Name
	} & Extension[Name] & Extension['Node']

type Identifier<Extension extends ExtensionInformation = Record<string, unknown>> =
	Node<"Identifier", Extension> & {
		name: string
	}

type Resource<Extension extends ExtensionInformation = Record<string, unknown>> =
...
type LanguageTag<Extension extends ExtensionInformation = Record<string, unknown>> =
...
const lint = <R extends Resource>(resource: R): R & Resource<{
...
const cache = <R extends Resource>(resource: R): R & Resource<{
	Node: { cache: Cache },
}> => resource as any
