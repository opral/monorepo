import type { Root as MdRoot, Content as MdContent, PhrasingContent as MdPhrasing, Text as MdText } from 'mdast'
...
export function mdWcToTiptap(ast: MdRoot): PMNode {
...
function astBlockToPM(node: MdContent): PMNode {
  switch (node.type) {
    ...
    case 'heading':
      return { type: 'heading', attrs: { level: (node as any).depth }, content: flattenInline((node as any).children || [], []) }
    case 'list': {
      const n = node as any
    ...
    case 'code': {
      const n = node as any
      return { type: 'codeBlock', attrs: { language: n.lang ?? null }, content: textContent(n.value || '') }
    }
    case 'thematicBreak':
    ...
    default:
      ...
      // @ts-ignore
      if ((node as any).children) return { type: 'paragraph', content: flattenInline((node as any).children, []) }
      return { type: 'paragraph', content: textContent('') }
  }
}
  ...
  for (const n of nodes) {
    switch (n.type) {
      ...
      case 'delete':
        ...
        break
      case 'inlineCode':
        out.push({ type: 'text', text: (n as any).value || '', marks: addMark(active, { type: 'code' }) })
        break
      case 'break':
