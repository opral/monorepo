...
export function tiptapToMdWc(doc: PMNode): MdRoot {
...
function pmBlockToAst(node: PMNode): any {
  switch (node.type) {
    ...
    case 'heading':
      return { type: 'heading', depth: node.attrs?.level || 1, children: pmInlineToMd(node.content || []) }
    case 'bulletList':
      return { type: 'list', ordered: false, children: (node.content || []).map(pmBlockToAst) }
    case 'orderedList': {
      const out: any = { type: 'list', ordered: true, children: (node.content || []).map(pmBlockToAst) }
