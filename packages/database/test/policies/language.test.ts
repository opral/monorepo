...
describe("policies/language", () => {
    test("Member can select language", async () => {
        const langauges = await supabase.from<definitions["language"]>("language")
            .select();
    ...
    test("Member can not upsert language", async () => {
        ...
        const langauge_upsert = await supabase.from<definitions["language"]>("language")
            .upsert({
                iso_code: "fr",
                project_id: project.data![0].id
            });
        const languages = await supabase.from<definitions["language"]>("language")
            .select()
            .match({
                iso_code: "fr",
                project_id: project.data![0].id
    ...
    test("Member can not delete language", async () => {
        ...
        const langauge_delete = await supabase.from<definitions["language"]>("language")
            .delete()
            .match({
                iso_code: "de",
                project_id: project.data![0].id
