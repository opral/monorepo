 ...
 */
export type RuleCollectionInitializer<
  RulesSettings extends Record<string, LintRuleInitializer>
> = (settings?: CollectionSettings<RulesSettings>) => LintRule[];

type CollectionSettings<
  RulesSettings extends Record<string, LintRuleInitializer>
> = {
  [Key in keyof RulesSettings]?:
    | Parameters<RulesSettings[Key]>[0]
    | Parameters<RulesSettings[Key]>;
};
 ...
 * ```
 */
export const createLintRuleCollection =
  <RulesSettings extends Record<string, LintRuleInitializer>>(
    rules: RulesSettings
  ): RuleCollectionInitializer<RulesSettings> =>
  (settings = {}) =>
    Object.entries(rules).map(([id, rule]) => {
      ...
      return rule(
        ...((Array.isArray(settingsForRule)
          ? settingsForRule
          : [settingsForRule]) as LintConfigArguments<any>)
      );
    });
