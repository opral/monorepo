...
export function createSqlSelectStateTool(args: { lix: Lix }) {
	const description = dedent`
			...
			State views:
			- state_all (all versions): entity_id, schema_key, file_id, plugin_key, snapshot_content (JSON), schema_version, created_at, updated_at, inherited_from_version_id, change_id, untracked, commit_id, version_id.

			Always select from state_all and include a WHERE clause on version_id to scope the results to the version you care about.
