...
- ✅ **Only CSS classes added**: Elements get `diff-create`, `diff-update`, or `diff-delete` classes
...
2. **`data-diff-key` only**: CSS classes added, structure preserved (atomic block diffing)
3. **`data-diff-key` + `data-diff-words`**: CSS classes + granular text spans (opt-in for text content)
