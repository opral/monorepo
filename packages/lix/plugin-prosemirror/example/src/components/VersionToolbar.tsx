...
import { createVersion, mergeVersion, switchVersion } from "@lix-js/sdk";
import {
	useLix,
	useQuery,
	useQueryTakeFirst,
...
const VersionToolbar: React.FC = () => {
	...
	const activeVersion = useQueryTakeFirstOrThrow(({ lix }) =>
		selectActiveVersion(lix),
	);
	const mainVersion = useQueryTakeFirstOrThrow(({ lix }) =>
		selectMainVersion(lix),
	);
	const activeAccount = useQueryTakeFirst(({ lix }) =>
		selectActiveAccount(lix),
	);
	const versions = useQuery(({ lix }) => selectVersions(lix));
	const scrollContainerRef = useRef<HTMLDivElement>(null);

	// Determine if current version is the main version
	const isMainVersion = mainVersion?.id === activeVersion?.id;
	...
	const handleMergeToMain = async () => {
		if (!activeVersion?.id || !mainVersion?.id) {
			return;
		}

		await mergeVersion({
			lix,
			source: { id: activeVersion.id },
		...
		>
			<div className="mode-tabs-scroll-container" ref={scrollContainerRef}>
				{versions.map((version) => (
					<button
						key={version.id}
						className={`mode-tab ${version.id === activeVersion?.id ? "active" : ""}`}
