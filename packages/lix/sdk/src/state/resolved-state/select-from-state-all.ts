...
export const RESOLVED_STATE_ALIAS = "internal_state_all_routed";
const RESOLVED_ALIAS = RESOLVED_STATE_ALIAS;
...
type ResolvedColumn = (typeof RESOLVED_COLUMNS)[number];

export type ResolvedStateRow = Record<ResolvedColumn, unknown>;

/**
 * Builds a query routed to the materialized state tables for the provided schema key.
 *
 * This mirrors the resolved-state logic used by the query preprocessor so that view
 * definitions and direct callers can avoid touching the legacy `internal_state_cache`
 * virtual table.
 *
 * @example
 * ```ts
 ...
 * ```
 */
export function selectFromStateAll(
	schemaKey: string
