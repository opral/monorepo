...
import {
	selectFromVtable,
	type SelectFromVtableContext,
} from "./select-from-vtable.js";

const buildVtableContext = (
	engine: Pick<LixEngine, "runtimeCacheRef">
): SelectFromVtableContext => ({
	existingCacheTables: getStateCacheTables({ engine }),
});
...
test("returns untracked entities", async () => {
	...
	const compiled = selectFromVtable(buildVtableContext(lix.engine!))
...
test("includes change metadata for inherited and live rows", async () => {
	...
	const baseCompiled = selectFromVtable(buildVtableContext(lix.engine!))
		.where("schema_key", "=", LixKeyValueSchema["x-lix-key"])
	...
	const inheritedCompiled = selectFromVtable(buildVtableContext(lix.engine!))
		.where("schema_key", "=", LixKeyValueSchema["x-lix-key"])
		.where("entity_id", "=", "metadata_key")
		.where("version_id", "=", activeVersion.version_id)
		.select([
...
test("exposes rows flagged as tombstones", async () => {
	...
	const compiled = selectFromVtable(buildVtableContext(engine))
		.where("schema_key", "=", LixKeyValueSchema["x-lix-key"])
		.where("entity_id", "=", "tombstone_key")
		.where("version_id", "=", "global")
...
test("unions cache tables when multiple schema keys are routed", async () => {
	...
	const compiled = selectFromVtable(buildVtableContext(lix.engine!))
		...
		.where("schema_key", "in", ["cache_schema_a", "cache_schema_b"])
		.compile();
...
test("skips metadata and writer joins when unused", async () => {
	const lix = await openLix({
		keyValues: [
			{
				key: "lix_deterministic_mode",
	...
	await updateStateCache({
		...
		changes: [
			{
				id: "prune-change",
				entity_id: "prune-entity",
				schema_key: LixKeyValueSchema["x-lix-key"],
				schema_version: LixKeyValueSchema["x-lix-version"],
				file_id: "file_prune",
				plugin_key: "test_plugin",
				snapshot_content: JSON.stringify({ value: "pruned" }),
				created_at: "1970-01-01T00:00:00.000Z",
			},
		],
		commit_id: "prune-commit",
		version_id: "global",
	});

	const compiled = selectFromVtable(buildVtableContext(lix.engine!))
		.where("schema_key", "=", LixKeyValueSchema["x-lix-key"])
		.where("entity_id", "=", "prune-entity")
		.select(["entity_id"])
		.compile();

	expect(compiled.sql).toContain("lix_internal_transaction_state");
	expect(compiled.sql).not.toContain("lix_internal_state_writer");
	expect(compiled.sql).not.toContain("change ch");
