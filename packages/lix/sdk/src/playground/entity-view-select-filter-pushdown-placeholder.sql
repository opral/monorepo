...
-- step expand sql (inline view, placeholders preserved)
SELECT
    "column_a",
    "column_b"
FROM (
...
-- step rewrite vtable select (cache swap + predicate pushdown, still parameterised)
SELECT
    "column_a",
    "column_b"
FROM (
    ...
    FROM (
        WITH RECURSIVE
            ...
            "state_candidates" AS (
                ...
                WHERE "cache"."schema_key" = 'test_schema'
                  AND json_extract("cache"."snapshot_content", '$.column_a') = ?1
        ...
        WHERE "rn" = 1
            ...
            AND json_extract("snapshot_content", '$.column_a') = ?1
            AND "version_id" = ?2
