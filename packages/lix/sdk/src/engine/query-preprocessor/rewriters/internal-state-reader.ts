/**
 * Routes `internal_state_reader` queries to the materialized cache tables.
 *
 * The reader exposes the fully resolved state (transactions + untracked +
 * cache + inherited rows).  When we detect a select that targets the
 * reader view and has a concrete schema filter we replace it with the
 * union query that stitches the underlying physical tables together.  This
 * mirrors the logic previously used for `internal_resolved_state_all`.
 *
 * @example
 * ```ts
