...
describe("analyzeShape", () => {
	test("collects filters and limit", () => {
		const sql = `SELECT json_extract(v.snapshot_content, '$.value') FROM lix_internal_state_vtable v
	...
	test("handles placeholders in IN lists", () => {
		const sql = `SELECT * FROM lix_internal_state_vtable v WHERE v.entity_id IN (?, :named)`;
		const shape = analyzeShape(tokenize(sql));
		expect(shape?.entityIds).toEqual([
	...
	test("detects writer_key selection", () => {
		const sql = `SELECT v.writer_key FROM lix_internal_state_vtable v WHERE v.schema_key = 'abc'`;
		const shape = analyzeShape(tokenize(sql));
		expect(shape).not.toBeNull();
	...
	test("selectsWriterKey defaults to false", () => {
		const sql = `SELECT v.entity_id FROM lix_internal_state_vtable v`;
		const shape = analyzeShape(tokenize(sql));
		expect(shape).not.toBeNull();
	...
	test("marks _pk references", () => {
		const sql = `SELECT v._pk FROM lix_internal_state_vtable v WHERE v.schema_key = 'abc'`;
		const shape = analyzeShape(tokenize(sql));
		expect(shape).not.toBeNull();
	...
	test("does not mark _pk when absent", () => {
		const sql = `SELECT * FROM lix_internal_state_vtable v WHERE v.schema_key = 'abc'`;
		const shape = analyzeShape(tokenize(sql));
		expect(shape).not.toBeNull();
	...
	test("analyzes filters nested inside subqueries", () => {
		...
		SELECT * FROM lix_internal_state_vtable v
	...
	test("analyzeShape returns the first shape", () => {
		const sql = `SELECT * FROM lix_internal_state_vtable v WHERE v.schema_key = 'one';
	SELECT * FROM lix_internal_state_vtable w WHERE w.schema_key = 'two';`;
		const shapes = analyzeShapes(tokenize(sql));
