...
describe("findInternalStateVtable", () => {
	test("finds plain table factor with alias", () => {
		const sql = "SELECT * FROM lix_internal_state_vtable s";
		...
		expect(segment).toBe("lix_internal_state_vtable s");
	});

	test("handles quoted table and alias", () => {
		const sql = 'SELECT * FROM "lix_internal_state_vtable" AS "State"';
		const tokens = tokenize(sql);
		const match = findTableFactor(tokens, "lix_internal_state_vtable");
		expect(match).toBeTruthy();
		...
		expect(match?.explicitAlias).toBe(true);
		const segment = sql.slice(match!.start, match!.end + 1);
		expect(segment).toBe('"lix_internal_state_vtable" AS "State"');
	});
	...
	test("falls back to table name when alias is missing", () => {
		...
		const match = findTableFactor(tokens, "lix_internal_state_vtable");
		expect(match).toBeTruthy();
		expect(match?.alias).toBe("lix_internal_state_vtable");
		expect(match?.explicitAlias).toBe(false);
		const segment = sql.slice(match!.start, match!.end + 1);
		expect(segment).toBe("lix_internal_state_vtable");
	});

	test("detects join usage", () => {
		const sql =
			"SELECT * FROM mock_other_table m JOIN lix_internal_state_vtable v ON v.id = m.id";
		const tokens = tokenize(sql);
		const match = findTableFactor(tokens, "lix_internal_state_vtable");
		expect(match).toBeTruthy();
		expect(match?.alias).toBe("v");
		const segment = sql.slice(match!.start, match!.end + 1);
		expect(segment).toBe("lix_internal_state_vtable v");
	});
	...
	test.each([
		...
		const sql = `SELECT * FROM lix_internal_state_vtable ${keyword} JOIN other_table o ON o.id = 1`;
		const tokens = tokenize(sql);
		const match = findTableFactor(tokens, "lix_internal_state_vtable");
		expect(match).toBeTruthy();
		expect(match?.alias).toBe("lix_internal_state_vtable");
	...
	test("general finder accepts arbitrary casing", () => {
		const sql = "SELECT * FROM lix_internal_state_vtable AS t";
		const tokens = tokenize(sql);
		const match = findTableFactor(tokens, "LIX_INTERNAL_STATE_VTABLE");
