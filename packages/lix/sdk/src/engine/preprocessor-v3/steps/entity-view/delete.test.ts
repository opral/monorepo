...
const getDeleteAst = (
	...
	const complete = trace.find((entry) => entry.step === "complete");
	if (!complete?.payload?.ast) {
		throw new Error("expected final AST in trace");
	}
	return complete.payload.ast as DeleteStatementNode;
};
...
test("rewrites delete with OR predicates", async () => {
	...
	const trace = rewritten.context?.trace ?? [];
	expect(
		trace.filter((entry) => entry.step === "rewrite_entity_view_delete")
	).toHaveLength(1);
