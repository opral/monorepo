...
import { expressionToSql } from "../../compile.js";
...
export const rewriteEntityViewUpdate: PreprocessorStep = (context) => {
	...
	if (node.node_kind !== "update_statement") {
		return node;
	}
	...
	const update = node as UpdateStatementNode;
	const viewName = extractTableName(update.target);
	if (!viewName) {
		return node;
	}
...
function buildEntityViewUpdate(args: BuildUpdateArgs): StatementNode | null {
	const {
		update,
		schema,
		variant,
	...
	const entityIdOverride = overridesObject.get("lixcol_entity_id");
...
function buildSnapshotExpression(args: {
	...
	for (const property of properties) {
		...
		if (assignment) {
			expressionSql = wrapSnapshotValue(definition, assignment.sql, true);
		} else {
