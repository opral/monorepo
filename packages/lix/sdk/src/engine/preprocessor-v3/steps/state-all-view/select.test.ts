...
test("projection includes metadata when explicitly selected", () => {
	...
	expect(sql).toContain('"v"."file_id" AS "file_id"');
	expect(sql).toContain('(SELECT json(metadata) FROM "change" WHERE "change"."id" = "v"."change_id") AS "metadata"');
});
...
test("prunes unused columns", () => {
	...
	const schemaKeyCount = sql.match(/"v"."schema_key" AS "schema_key"/g)?.length ?? 0;
	const snapshotCount = sql.match(/"v"."snapshot_content" AS "snapshot_content"/g)?.length ?? 0;
