...
test("timestamp returns deterministic values when deterministic mode is enabled", async () => {
	...
	const t1 = await getTimestamp({ lix });
	const t2 = await getTimestamp({ lix });
	const t3 = await getTimestamp({ lix });
...
test("timestamp is persisted across lix instances", async () => {
	...
	const t1 = await getTimestamp({ lix: lix1 });
	const t2 = await getTimestamp({ lix: lix1 });
	const t3 = await getTimestamp({ lix: lix1 });
	...
	const t4 = await getTimestamp({ lix: lix2 });
...
test("timestamp advances correctly with many operations", async () => {
	...
	for (let i = 0; i < 100; i++) {
		timestamps.push(await getTimestamp({ lix }));
	}
