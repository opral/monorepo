...
describe("entity view select synthesis", () => {
	...
	test("generates base, _all, and _history selects for stored schema", async () => {
		...
		expect(base).toContain("FROM lix_internal_state_vtable");
		expect(base).toContain("schema_key = 'unit_test_schema'");
		expect(base).toContain(
			"AND v.version_id = (SELECT version_id FROM active_version)"
		);
		expect(base).toContain("json_extract(v.snapshot_content, '$.id') AS id");
		expect(base).toContain(
			"json_extract(v.snapshot_content, '$.label') AS label"
		);
		expect(base).toContain("v.plugin_key AS lixcol_plugin_key");
		expect(base).toContain("v.snapshot_content IS NOT NULL");

		const all = map.get("unit_test_schema_all")!;
		expect(all).toContain("FROM lix_internal_state_vtable");
		expect(all).toContain("version_id AS lixcol_version_id");
		expect(all).toContain("v.snapshot_content IS NOT NULL");
	...
	test("selecting from stored schema view returns rows via preprocessor", async () => {
		...
		const preprocess = await createPreprocessor({ engine: lix.engine! });
