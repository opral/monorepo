...
test("random works in deterministic mode", async () => {
	...
	const sequence2 = [];
	for (let i = 0; i < 10; i++) {
		sequence2.push(await random({ lix: lix2 }));
	}
...
test("deterministic mode produces same sequence with same seed", async () => {
	...
	const sequence1 = [];
	for (let i = 0; i < 5; i++) {
		sequence1.push(await random({ lix: lix1 }));
	}
	...
	for (let i = 0; i < 5; i++) {
		sequence1.push(await random({ lix: lix1b }));
	}
...
test("random state persists across blob operations", async () => {
	...
	const fullSequence = [];
	for (let i = 0; i < 6; i++) {
		fullSequence.push(await random({ lix: lix1 }));
	}
	...
	const beforeBlob = [];
	for (let i = 0; i < 3; i++) {
		beforeBlob.push(await random({ lix: lix2 }));
	}
	...
	const afterBlob = [];
	for (let i = 0; i < 3; i++) {
		afterBlob.push(await random({ lix: lix3 }));
	}
...
test("random works after enabling deterministic mode", async () => {
	...
	const nonDeterministicValue = await random({ lix });
	...
	const sequence1 = [];
	for (let i = 0; i < 5; i++) {
		sequence1.push(await random({ lix }));
	}
