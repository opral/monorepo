...
Open a lix with `openLix()` and provide plugins for the file types you are going to use.
...
- Filter by `lixcol_root_commit_id` (and typically `lixcol_depth = 0`) to scope history
  to a single checkpoint and avoid mixing multiple timelines
