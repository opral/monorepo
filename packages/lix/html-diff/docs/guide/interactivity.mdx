...
<p data-diff-key="intro" data-diff-mode="words" class="diff-modified">
  ...
  <span class="diff-removed">users</span><span class="diff-added">developers</span>.
</p>
```
...
const modifiedElements = container.querySelectorAll('.diff-modified[data-diff-key]');
const addedElements = container.querySelectorAll('.diff-added[data-diff-key]');
const removedElements = container.querySelectorAll('.diff-removed[data-diff-key]');
...
export function InteractiveHtmlDiff() {
  ...
  useEffect(() => {
    ...
    const modifiedElements = ref.current.querySelectorAll('.diff-modified[data-diff-key]');
