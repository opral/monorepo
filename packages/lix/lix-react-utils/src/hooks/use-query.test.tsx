...
test('useQuery throws error when used outside LixProvider', () => {
  ...
  expect(result.error?.message).toBe('useQuery must be used inside <LixProvider>.')
})
...
test('useQuery returns loading state initially', async () => {
  ...
  const { result } = renderHook(
    () => useQuery(db => db.selectFrom('key_value').selectAll().where('key', 'like', 'test_%')),
    { wrapper }
  )
...
test('useQuery updates when data changes', async () => {
  ...
  const { result } = renderHook(
    () => useQuery(db => db.selectFrom('key_value').selectAll().where('key', 'like', 'react_test_%')),
    { wrapper }
  )
...
test('useQuery accepts query builder directly', async () => {
  const lix = await openLix({})
  const queryBuilder = lix.db.selectFrom('key_value').selectAll().where('key', 'like', 'direct_test_%')
...
test('useQuery multiple subscriptions work independently', async () => {
  ...
  const { result: result1 } = renderHook(
    () => useQuery(db => db.selectFrom('key_value').selectAll().where('key', 'like', 'multi_1_%')),
    { wrapper }
  )

  const { result: result2 } = renderHook(
    () => useQuery(db => db.selectFrom('key_value').selectAll().where('key', 'like', 'multi_2_%')),
    { wrapper }
  )
...
test('useQueryFirst returns undefined for empty results', async () => {
  ...
  const { result } = renderHook(
    () => useQueryFirst(db => db.selectFrom('key_value').selectAll().where('key', '=', 'non_existent')),
    { wrapper }
  )
...
test('useQueryFirstOrThrow throws error for empty results', async () => {
  ...
  expect(result.error).toBeInstanceOf(Error)
  expect(result.error?.message).toBe('useQueryFirstOrThrow: query returned no rows')
