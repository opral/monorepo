...
<div class="p-8">
	<DataTable {headers} rows={rows()}>
		<Toolbar>
			...
			<ToolbarContent>
				<ToolbarSearch placeholder="Search {name}" />
				<Button on:click={() => (showAddEntityModal = true)}>Add {name}</Button>
			</ToolbarContent>
		</Toolbar>
...
<!-- TODO -->
{#if showAddEntityModal && name == 'organization'}
	<OrganizationModal
		open={true}
		...
		organizationName=""
	/>
{:else if showAddEntityModal && name == 'project'}
