...
export default async function runExample(console: any) {
  ...
  // SECTION-START "plugin-diffs"
  const plugin = (await lix.plugin.getAll()).find((p: any) => p.key === "csv_plugin");
  ...
  if (plugin?.diffComponent) {
    ...
    console.log("Plugin diff component available:", !!plugin.diffComponent);
  } else {
    console.log("No diff component found for CSV plugin");
  }
  // SECTION-END "plugin-diffs"

  // SECTION-START "json-file-diff"
  ...
  // SECTION-END "json-file-diff"

  // SECTION-START "entity-diff"
  ...
  if (currentEntities.length >= 2) {
    ...
    console.log(`Entity 1: ${currentEntities[0].entity_id} (${currentEntities[0].schema_key})`);
    console.log(`Entity 2: ${currentEntities[1].entity_id} (${currentEntities[1].schema_key})`);
