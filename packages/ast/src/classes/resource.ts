...
export class Resource extends FluentResource {
	...
	updateAttribute(args: {
		...
		if (indexOfMessage === -1) {
			return Result.err(
				Error(`Message with id '${args.messageId}' does not exist.`)
			);
		}
		const indexOfAttribute = (
			cloned.body[indexOfMessage] as Message
		).attributes.findIndex((attribute) => attribute.id.name === args.id);
		if (indexOfAttribute === -1) {
			return Result.err(
				Error(
					`Attribute with id '${args.id}' does not exist for the message with id '${args.messageId}'.`
				)
			);
		}
		merge(
			(cloned.body[indexOfMessage] as Message).attributes[indexOfAttribute],
			args.with
		);
		return Result.ok(cloned);
	}
	...
	upsertAttribute(args: {
		...
		const cloned = cloneDeep(this);
		const attribute = cloned.getAttribute({
			id: args.attribute.id.name,
			messageId: args.messageId,
		});
		if (attribute) {
	...
	deleteAttribute(args: {
		...
		const cloned = cloneDeep(this);
		for (const message of cloned.body.filter(
			(entry) => entry.type === "Message"
		)) {
			if ((message as Message).id.name === args.messageId) {
				const removed = remove(
					(message as Message).attributes,
					(attribute) => attribute.id.name === args.id
				);
				if (removed.length === 0) {
					return Result.err(
						Error(
							`Attribute with id ${args.id} does not exist for message with id '${args.messageId}'.`
						)
					);
				}
				return Result.ok(cloned);
		...
		return Result.err(
			Error(
				`The attributes parent message with id '${args.id}' does not exist.`
			)
		);
	}
	...
	createMessage(message: Message): Result<Resource, Error> {
		if (this.includesMessage({ id: message.id.name })) {
			return Result.err(
				Error(`Message with id '${message.id}' already exists.`)
			);
		}
