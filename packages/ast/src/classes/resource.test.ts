...
describe("create", () => {
	describe("Message()", () => {
		const resource = new Resource([
			Message.from({ id: "first-message", value: "this is my test" }).unwrap(),
		]);
		...
		it("should create a new message with attributes", () => {
			const newResource = resource
				...
				.unwrap();
			expect(newResource.body.length).toBe(2);
			expect((newResource.body[1] as Message).attributes[0].id.name).toBe(
				"new-attribute"
			);
		});
		...
		it("should be immutable", () => {
			...
			resource
				.createMessage(
					Message.from({ id: "new-message", value: "new pattern" }).unwrap()
				)
				.unwrap();
			resource
				.createMessage(
...
describe("get", () => {
	describe("Message()", () => {
		...
		it("should return the message", () => {
			expect(resource.getMessage({ id: "the-message" })).toEqual(
				resource.body[0]
			);
		});
	...
	describe("Attribute()", () => {
		...
		it("should return the attribute", () => {
			expect(
				resource.getAttribute({ messageId: "the-message", id: "the-attribute" })
			).toEqual((resource.body[0] as Message).attributes[0]);
		});
	...
	describe("Message()", () => {
		...
		it("should update the value (pattern)", () => {
			const newResource = resource
				...
				.unwrap();
			expect((newResource.body[0] as Message).value?.elements[0].value).toBe(
				"updated text"
			);
		});
		...
		it("should update the attributes", () => {
			const newResource = resource
				...
				.unwrap();
			expect(
				(newResource.body[0] as Message).attributes[0].value.elements[0].value
			).toBe("the attribute text");
		});
		...
		it("should be immutable", () => {
			...
			const _ = resource
				...
				.unwrap();
			expect((resource.body[0] as Message).value?.elements[0].value).toBe(
				"this is my test"
			);
		});
	});
	describe("Attribute()", () => {
		...
		it("should be immutable", () => {
			...
			const _ = resource
				...
				.unwrap();
			expect(
				(resource.body[0] as Message).attributes[0].value.elements[0].value
			).toBe("some text pattern");
		});
	});
});
...
describe("delete", () => {
	...
	describe("Attribute()", () => {
		...
		it("should delete the attribute", () => {
			const newResource = resource
				.deleteAttribute({ messageId: "the-message", id: "the-attribute" })
				.unwrap();
...
describe("upsert", () => {
	describe("Attribute()", () => {
		...
		it('should create a new message containing the attribute if the "parent message" does not exist', () => {
			...
			expect(
				(newResource.body[1] as Message).attributes[0].value.elements[0].value
			).toEqual("a pattern");
		});
	});
});
