...
describe("get", async () => {
  ...
  it("should return an object, not an array", async () => {
    ...
    const mockMessageInit = createMessage("first-message", {
      en: "Hello World",
    });
  ...
  it.skip("mutating the returned value should not affect subsequent return values", async () => {
    ...
    (
      message1.variants.find((v) => v.languageTag === "en")!
        .pattern![0]! as Text
    ).value = "Hello World 2";
    ...
    expect(
      (
        message2.variants.find((v) => v.languageTag === "en")!
          .pattern![0]! as Text
      ).value,
    ).toBe("Hello World");
  });
...
describe("getAll", async () => {
  ...
  it("should return all message objects", async () => {
    ...
    const mockMessage2 = createMessage("second-message", {
      en: "Hello World 2",
    });
  ...
  it.skip("mutating the returned value should not affect subsequent return values", async () => {
    ...
    (
      Object.values(messages1!)[0]!.variants.find(
        (v) => v.languageTag === "en",
      )!.pattern![0]! as Text
    ).value = "Hello World 2";
    ...
    expect(
      (
        Object.values(query.getAll()!)[0]!.variants.find(
          (v) => v.languageTag === "en",
        )!.pattern![0]! as Text
      ).value,
    ).toBe("Hello World");
  ...
  describe("get", async () => {
    ...
    it("should react to `update`", async () => {
      await createRoot(async () => {
        ...
        expect(
          (
            message?.variants.find((variant) => variant.languageTag === "en")
              ?.pattern[0] as Text
          ).value,
        ).toBe("before");
    ...
    it("should react to `upsert`", async () => {
      await createRoot(async () => {
        ...
        query.upsert({
          where: { id: "1" },
          data: createMessage("1", { en: "before" }),
        });
        expect(message).toBeDefined();
        expect(
          (
  ...
  describe("getAll", async () => {
    ...
    it("should react to `update`", async () => {
      await createRoot(async () => {
        ...
        expect(
          (
            Object.values(messages!)![0]!.variants.find(
    ...
    it("should not mutate messages signal outside the query when using the query", async () => {
      const [inputMessages] = createSignal<Message[]>([
        createMessage("1", { en: "before" }),
      ]);
...
it("instances should not share state", async () => {
  await createRoot(async () => {
    const query1 = await awaitableCreateMessageQuery(() => [
      createMessage("1", { en: "before" }),
    ]);
    ...
    expect(
      (
        message1!.variants.find((variant) => variant.languageTag === "en")!
          .pattern[0]! as Text
      ).value,
    ).toBe("after");
    expect(
      (
        message2!.variants.find((variant) => variant.languageTag === "en")!
          .pattern[0]! as Text
      ).value,
    ).toBe("before");
