...
export function createMessagesQuery({
  ...
  const setDelegate = (
    newDelegate: MessageQueryDelegate | undefined,
    onLoad: boolean,
  ) => {
  ...
  createEffect(() => {
    ...
    if (!resolvedPluginApi.loadMessages) {
      onInitialMessageLoadResult(
        new Error("no loadMessages in resolved Modules found"),
      );
      return;
    }
    loadMessagesViaPlugin(
  ...
  const get = (args: Parameters<MessageQueryApi["get"]>[0]) =>
    index.get(args.where.id);

  const getByDefaultAlias = (
    alias: Parameters<MessageQueryApi["getByDefaultAlias"]>[0],
  ...
  return {
    ...
    getByDefaultAlias: Object.assign(getByDefaultAlias, {
      subscribe: (
        ...
        callback: Parameters<
          MessageQueryApi["getByDefaultAlias"]["subscribe"]
        >[1],
      ) =>
...
async function loadMessagesViaPlugin(
  ...
  try {
    lockTime = await acquireFileLock(
    ...
    batch(() => {
      for (const loadedMessage of loadedMessages) {
        ...
        const currentMessages = [...messages.values()]
          ...
          .filter(
            (message: any) =>
              (experimentalAliases ? message.alias["default"] : message.id) ===
              loadedMessage.id,
          );
        ...
        if (currentMessages.length > 1) {
          ...
          throw new Error(
            "more than one message with the same id or alias found ",
          );
          ...
          if (
            messageState.messageLoadHash[loadedMessageClone.id] ===
            importedEnecoded
          ) {
          ...
          if (loadedMessageClone.alias["default"]) {
            aliaseToMessageMap.set(
              loadedMessageClone.alias["default"],
              loadedMessageClone,
            );
          }
          // NOTE could use hash instead of the whole object JSON to save memory...
...
async function saveMessagesViaPlugin(
  ...
  messageState.currentSaveMessagesViaPlugin = (async function () {
    ...
    let messageDirtyFlagsBeforeSave:
      | typeof messageState.messageDirtyFlags
      | undefined;
    ...
    try {
      ...
      if (Object.keys(messageState.messageDirtyFlags).length == 0) {
        debug(
          "save was skipped - no messages marked as dirty... releasing lock again",
        );
        messageState.isSaving = false;
      ...
      for (const message of currentMessages) {
        ...
        if (settingsValue.experimental?.aliases) {
          fixedExportMessage.id =
            fixedExportMessage.alias["default"] ?? fixedExportMessage.id;
        }
      ...
      for (const [messageId, messageHash] of Object.entries(
        saveMessageHashes,
      )) {
      ...
      if (messageState.sheduledLoadMessagesViaPlugin) {
        debug(
          "saveMessagesViaPlugin calling queued loadMessagesViaPlugin to share lock",
        );
...
const makeTrulyAsync = <T>(fn: MaybePromise<T>): Promise<T> =>
  (async () => fn)();
