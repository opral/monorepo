...
export const resolvePlugins: ResolvePluginsFunction = async (args) => {
  ...
  for (const plugin of args.plugins) {
    ...
    if (
      ...
      result.errors.push(
        new PluginLoadMessagesFunctionAlreadyDefinedError({ id: plugin.id }),
      );
    }
    ...
    if (
      ...
      result.errors.push(
        new PluginSaveMessagesFunctionAlreadyDefinedError({ id: plugin.id }),
      );
    }
      ...
      if (error) {
        ...
        result.errors.push(
          new PluginReturnedInvalidCustomApiError({
            id: plugin.id,
            cause: new Error(
              `The return value must be an object. Received "${typeof customApi}".`,
            ),
          }),
        );
      }
