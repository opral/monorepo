...
export async function openStore(args: {
  ...
  async function load() {
    ...
    const index = new Map<string, MessageBundle>(
...
export async function readJSON(args: {
  ...
  try {
    const file = await args.nodeishFs.readFile(args.filePath, {
      encoding: "utf-8",
    });
...
export async function writeJSON(args: {
  ...
  try {
    ...
    const output = injectJSONNewlines(
      JSON.stringify(
        args.messages.map((bundle) => addSlots(bundle, args.locales)),
      ),
    );
...
async function createDirectoryIfNotExits(
  path: string,
  nodeishFs: NodeishFilesystem,
) {
  try {
