...
describe("initialization", () => {
  it("should throw if projectPath is not an absolute path", async () => {
    ...
    expect(result.error?.message).toBe(
      'Expected an absolute path but received "relative/path".',
    );
  ...
  it("should generate projectId on missing projectid", async () => {
    ...
    expect(newId).toBe(
      "aef225403be8b526dfb492a4617fd59d8181e8fef2c7f4aff56ab299046e36ed",
    );
  ...
  it.skipIf(() => process.platform !== "win32")(
    "should resolve from a windows path",
    async () => {
      const repo = await mockRepo();
      const fs = repo.nodeishFs;
      await fs.mkdir("C:\\Users\\user\\project.inlang", { recursive: true });
      await fs.writeFile(
        "C:\\Users\\user\\project.inlang\\settings.json",
  ...
  describe("settings", () => {
    it("should return an error if settings file is not found", async () => {
      ...
      expect(project.errors()![0]).toBeInstanceOf(
        ProjectSettingsFileNotFoundError,
      );
    });
    ...
    it("should not re-write the settings to disk when initializing", async () => {
      const repo = await mockRepo();
      const fs = repo.nodeishFs;
      const settingsWithDifferentFormatting = JSON.stringify(
        settings,
        undefined,
        4,
      );
      ...
      await fs.writeFile(
        "/user/project.inlang/settings.json",
        settingsWithDifferentFormatting,
      );
...
describe("functionality", () => {
  describe("settings", () => {
    ...
    it("should set a new settings", async () => {
      ...
      expect(project.settings()!.languageTags).toStrictEqual([
        "en",
        "de",
        "fr",
      ]);
  ...
  describe("setSettings", () => {
    ...
    it("should write settings to disk", async () => {
      ...
      const before = await fs.readFile("/user/project.inlang/settings.json", {
        encoding: "utf-8",
      });
      ...
      const after = await fs.readFile("/user/project.inlang/settings.json", {
        encoding: "utf-8",
      });
  ...
  describe("installed", () => {
    ...
    it("should return lint reports if disabled is not set", async () => {
      ...
      const _mockPlugin: Plugin = {
        ...
        loadMessages: () => [
          { id: "some-message", alias: {}, selectors: [], variants: [] },
        ],
      ...
      expect(
        await project.query.messageLintReports.getAll.settled(),
      ).toHaveLength(1);
      expect(
        (await project.query.messageLintReports.getAll.settled())?.[0]?.ruleId,
      ).toBe(_mockLintRule.id);
    ...
    it("should return lint reports for a single message", async () => {
      ...
      expect(
        await project.query.messageLintReports.get({
          where: { messageId: "some-message" },
        }),
      ).toHaveLength(1);
    });
  });
  ...
  describe("messages", () => {
    it("should return the messages", async () => {
      ...
      expect(Object.values(project.query.messages.getAll())).toEqual(
        exampleMessages,
      );
    });
  });
  ...
  describe("query", () => {
    ...
    it("should pass all messages, regardless of which message changed, to saveMessages()", async () => {
      ...
      project.query.messages.create({
        data: createMessage("fourth", { en: "fourth message" }),
      });
      ...
      project.query.messages.create({
        data: createMessage("fifth", { en: "fifth message" }),
      });
  ...
  describe("watcher", () => {
    it("changing files in resources should trigger callback of message query", async () => {
      ...
      const getMessages = async (customFs: NodeishFilesystemSubset) => {
        const file = await customFs.readFile("./messages.json", {
          encoding: "utf-8",
        });
