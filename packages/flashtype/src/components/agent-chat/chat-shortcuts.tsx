...
export function ChatShortcuts({
	...
	return (
		<div
			className={[
				"flex flex-wrap items-center gap-2 rounded-md border bg-muted/30 px-2 py-1 font-mono text-[11px] text-muted-foreground",
				className ?? "",
			].join(" ")}
		>
			{hints.map((h, i) => (
				<div key={`${h.key}-${i}`} className="inline-flex items-center gap-1">
					<kbd className="rounded border bg-background px-1 py-[1px] text-[10px] font-medium">
						{h.key}
					</kbd>
					<span>{h.label}</span>
				</div>
