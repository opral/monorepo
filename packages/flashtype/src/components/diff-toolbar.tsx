...
export function DiffToolbar() {
	...
	const [explicitSourceId, setDiffSource] = useKeyValue(
	...
	const activeSource = useQueryTakeFirstOrThrow(({ lix }) =>
		lix.db
	...
	const explicitSource = useQueryTakeFirst(({ lix }) =>
		lix.db
			.selectFrom("version")
			.where("id", "=", (explicitSourceId as any) ?? "__none__")
			.selectAll()
			.limit(1),
	);

	const source = explicitSource ?? activeSource;
	...
	const added = Array.isArray(diffRows)
		? diffRows.filter(
				(d: any) => d.status === "added" || d.status === "modified",
			).length
		: 0;
