...
export const outOfOrderSequenceSimulation: SimulationTestDef = {
	...
	setup: async (lix) => {
		...
		const createShuffledMapping = (maxSequences: number): Map<number, number> => {
		...
		vi.spyOn(sequenceModule, "nextDeterministicSequenceNumber").mockImplementation(() => {
			...
			const shuffledSequence = shuffleMapping.get(normalSequence) ?? normalSequence;
