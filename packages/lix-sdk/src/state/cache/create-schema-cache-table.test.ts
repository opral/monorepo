...
import { createSchemaCacheTable, schemaKeyToCacheTableName } from "./create-schema-cache-table.js";
...
test("createSchemaCacheTable creates table with core indexes and is idempotent", async () => {
  ...
  const idxCount = idxRows2.filter((r) => String(r?.[0] || "").startsWith(`idx_${tableName}_`)).length;
