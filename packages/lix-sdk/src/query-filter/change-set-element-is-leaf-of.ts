 ...
 *
 * A change is considered a **leaf at point** if no other change in the ancestry of the target
 * change set redefines the same entity.
 *
	...
	return () =>
		sql<SqlBool>`
			NOT EXISTS (
				...
				JOIN change_set_element AS newer_cse ON newer_cse.change_id = newer.id
				WHERE
