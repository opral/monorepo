...
test.todo(
	"it should update the branch pointers for non-conflicting changes and insert detected conflicts",
	async () => {
...
test.todo(
	"it should automatically detect a conflict if a change exists that differs updates in both branches despite having a common ancestor",
	async () => {
		const lix = await openLixInMemory({});
