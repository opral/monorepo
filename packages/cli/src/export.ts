...
async function entry() {
	...
	program.version('0.1.0').option('-p, --project', 'project id from dashboard').parse(args);
	...
	if (translations.data === null || translations.error) {
		console.error('Something went wrong fetching the translations. Does the project exist?');
		return;
	}
