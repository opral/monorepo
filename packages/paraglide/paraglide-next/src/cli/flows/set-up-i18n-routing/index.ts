...
> = async (ctx) => {
  for await (const relativePath of globIterate("**/*.{ts,tsx,js,jsx,mjs}", {
    ...
    const content = await ctx.repo.nodeishFs.readFile(path, {
      encoding: "utf-8",
    });
    const newContent = replaceNextNavigationImports(
      replaceNextLinkImports(content),
    );
...
export function replaceNextNavigationImports(content: string) {
  ...
  const regex =
    /import\s+{(?<identifiersString>[^}]*)}\s+from\s*("|')next\/navigation("|')/g;
  ...
  const identifiers: [string, string | undefined][] = identifiersString
    ...
    .filter(Boolean)
    .map((part) => {
      return part.split(" as ").map((i) => i.trim()) as [
        string,
        string | undefined,
      ];
    });
