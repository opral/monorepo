...
> = async (ctx) => {
	...
	return { ...ctx, srcRoot, nextConfigFile: nextConfigFile, packageJsonPath, typescript, router }
}
...
const createI18nFile: CliStep<
	...
	unknown
> = async (ctx) => {
	const i18nFilePath = path.join(ctx.srcRoot, ctx.typescript ? "lib/i18n.ts" : "lib/i18n.js")
...
`
	...
	const file = ctx.typescript ? TypescriptTemplate : JavascriptTemplate
...
const createMiddlewareFile: CliStep<
	{ srcRoot: string; repo: Repository; logger: Logger; typescript: boolean },
	unknown
> = async (ctx) => {
	const middlewareFilePath = path.join(
		ctx.srcRoot,
		ctx.typescript ? "middleware.ts" : "middleware.js"
	)
...
const addLanguageProvider: CliStep<
	...
	unknown
> = async (ctx) => {
	const layoutFilePath = path.join(ctx.srcRoot, ctx.typescript ? "app/layout.tsx" : "app/layout.js")
