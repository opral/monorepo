...
describe("getCanonicalPath", () => {
  ...
  it("get's the canonical path for translated paths", () => {
    expect(getCanonicalPath("/de/uebersetzt", "de")).toBe(
      "/canonical-translated",
    );
    expect(getCanonicalPath("/en/translated", "en")).toBe(
      "/canonical-translated",
    );
    expect(getCanonicalPath("/de-CH/uebersetzt", "de-CH")).toBe(
      "/canonical-translated",
    );
  });

  it("get's the canonical path for translated paths with params", () => {
    expect(getCanonicalPath("/de/uebersetzt/1", "de")).toBe(
      "/canonical-translated/1",
    );
    expect(getCanonicalPath("/en/translated/1", "en")).toBe(
      "/canonical-translated/1",
    );
    expect(getCanonicalPath("/de-CH/uebersetzt/1", "de-CH")).toBe(
      "/canonical-translated/1",
    );
  });

  it("does not get confused by a path that starts with a language tag", () => {
    expect(getCanonicalPath("/de-something-else", "de")).toBe(
      "/de-something-else",
    );
  });
});
...
describe("getLocalisedPath", () => {
  it("adds a language prefix if there isn't one", () => {
    expect(getLocalisedHref("/some/path", "de", false)).toEqual({
    ...
    expect(getLocalisedHref("/", "de-CH", false)).toEqual({
      pathname: "/de-CH",
    });
  ...
  it("does not add a language prefix if the new locale is the source language tag", () => {
    expect(getLocalisedHref("/some/path", "en", false)).toEqual({
      pathname: "/some/path",
    });
  });
