...
const { getCanonicalPath, getLocalisedUrl, resolveLocale } = DomainStrategy<
  "en" | "de" | "de-CH"
>({
  domains: {
...
describe("getLocalisedUrl", () => {
  it("retuns the path as-is if it's not a language switch", () => {
    expect(getLocalisedUrl("/some/path", "de", false)).toEqual({
      pathname: "/some/path",
    });
    expect(getLocalisedUrl("/some/path", "de-CH", false)).toEqual({
      pathname: "/some/path",
    });
  });
