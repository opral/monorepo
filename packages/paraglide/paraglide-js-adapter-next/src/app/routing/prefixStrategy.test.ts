...
describe("getLocalisedPath", () => {
	it("adds a language prefix if there isn't one", () => {
		expect(getLocalisedHref("/", "de", { isLocaleSwitch: false })).toBe("/de")
		expect(getLocalisedHref("/", "de-CH", { isLocaleSwitch: false })).toBe("/de-CH")
		expect(getLocalisedHref("/", "en", { isLocaleSwitch: false })).toBe("/")
		expect(getLocalisedHref("/some/path", "de", { isLocaleSwitch: false })).toBe("/de/some/path")
		expect(getLocalisedHref("/some/path", "de-CH", { isLocaleSwitch: false })).toBe(
			"/de-CH/some/path"
		)
	})

	it("does not add a language prefix if the new locale is the source language tag", () => {
		expect(getLocalisedHref("/some/path", "en", { isLocaleSwitch: false })).toBe("/some/path")
	})

	it("does not localise excluded paths", () => {
		expect(getLocalisedHref("/api/some/path", "de", "en", "")).toBe("/api/some/path")
	})

	it("get's translated paths", () => {
		expect(getLocalisedHref("/canonical-translated", "de", "de", "")).toBe("/de/uebersetzt")
		expect(getLocalisedHref("/canonical-translated", "en", "de", "")).toBe("/translated")
		expect(getLocalisedHref("/canonical-translated", "de-CH", "de", "")).toBe("/de-CH/uebersetzt")
	})

	it("get's translated paths with params", () => {
		expect(getLocalisedHref("/canonical-translated/1", "de", { isLocaleSwitch: false })).toBe(
			"/de/uebersetzt/1"
		)
		expect(getLocalisedHref("/canonical-translated/1", "en", { isLocaleSwitch: false })).toBe(
			"/translated/1"
		)
		expect(getLocalisedHref("/canonical-translated/1", "de-CH", { isLocaleSwitch: false })).toBe(
			"/de-CH/uebersetzt/1"
		)
	})
})
