...
const createNestedObject = (base: BaseTranslation, names: string[], text: string) => {
	if (names.length === 1) {
		base[names[0]] = text;
	} else {
		...
		base[names[0]] = createNestedObject(base[names[0]] as BaseTranslation, names.slice(1), text);
	}
	return base;
};
...
const updateTranslations = async () => {
	...
	for (const t of translations.data ?? []) {
		...
		localeMapping.translations = createNestedObject(localeMapping.translations, nest, t.text);
	}
