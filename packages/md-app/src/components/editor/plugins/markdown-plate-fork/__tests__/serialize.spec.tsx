...
describe('Markdown-Plate-Fork Plugin Serialization Tests', () => {
  ...
  it('should handle custom nodes', () => {
    const editorValue = (
      <fragment>
        <hp>
          Text with <htext color="blue">custom colored</htext> mark.
        </hp>
        <element type="frontmatter" value="title: Test Document\nauthor: Test User">
          <htext />
        </element>
      </fragment>
    );

    // Just check that serialization completes without errors for custom nodes
    const result = serializeToMarkdown(editor, editorValue);
