...
export const footer = html`
	...
	<div class="w-full max-w-2xl px-4 mx-auto my-16">
		...
		<ul class="leading-[1.7] pl-0 list-none flex gap-2">
			${footerLinks.map(
				(useCase) => html`
					<li>
						<a
							class="text-slate-600 underline decoration-slate-300 font-medium hover:decoration-slate-950"
							href="${useCase.link}"
							>${useCase.title}</a
						>
					</li>
		...
		<a
			...
			target="_blanc"
			class="mt-2 w-fit text-[16px] px-4 py-3 text-slate-700 font-medium rounded-lg bg-slate-100 hover:bg-slate-200 border-none cursor-pointer flex items-center gap-2 no-underline"
		>
			<svg
				role="img"
				width="24"
