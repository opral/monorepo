...
describe("main workflow", () => {
  ...
  it("should throw errors directly", async () => {
    try {
      repository = await openRepository(
        "https://github.com/inlang/does-not-exist",
        {
  ...
  it("supports local repos", async () => {
    ...
    const snapshot = JSON.parse(
      readFileSync(resolve(__dirname, "../mocks/ci-test-repo.json"), {
        encoding: "utf-8",
      })
    );
    ...
    const repository: Awaited<ReturnType<typeof openRepository>> =
      await openRepository(repoUrl!, {
        nodeishFs: fs,
        branch: "test-symlink",
      });
  ...
  it("usees the lix custom commit for the whitelistesd ci test repo", () => {
    expect(repository._experimentalFeatures).toStrictEqual({
      lixCommit: true,
      lazyClone: true,
    });
  });
  ...
  it("can open repo with lazy clone/checkout", async () => {
    const lazyRepo = await openRepository(
      "https://github.com/opral/ci-test-repo",
      {
        branch: "test-symlink",
        nodeishFs: createNodeishMemoryFs(),
    ...
    expect(
      await lazyRepo.nodeishFs.readFile("./.npmrc", { encoding: "utf-8" })
  ...
  it("uses nested gitignore files correctly", async () => {
    ...
    const repo: Awaited<ReturnType<typeof openRepository>> =
      await openRepository("file:///", {
        nodeishFs: fs,
        experimentalFeatures: { lazyClone: true, lixCommit: true },
        branch: "test-symlink",
      });
    ...
    try {
      ...
      expect(err.message).toBe(
        "could not empty the workdir, uncommitted changes"
      );
    }
    ...
    await repo.nodeishFs.mkdir("/static/test/nested/deep/deeper", {
      recursive: true,
    });

    await repo.nodeishFs.writeFile(
    ...
    const status = (
      await repo.statusList({ includeStatus: ["materialized", "ignored"] })
  ...
  it("finds initial commit of repo", async () => {
    ...
    const snapshot = JSON.parse(
      readFileSync(
        resolve(__dirname, "../mocks/ci-test-repo-no-shallow.json"),
        {
          encoding: "utf-8",
        }
      )
