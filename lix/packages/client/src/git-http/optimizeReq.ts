...
export async function optimizeReq(
  ...
  if (
    url.endsWith("info/refs?service=git-upload-pack") &&
    gitConfig.addRefs !== undefined
  ) {
    const uploadPackUrl = url.replace(
  ...
  if (method === "POST") {
    ...
    const newBody = rawLines
      .map((updatedRawLine: any) => encodePackLine(updatedRawLine))
      .join("");
...
export async function optimizeRes({
  ...
  if (
    !origUrl.endsWith("info/refs?service=git-upload-pack") ||
    statusCode !== 200
  ) {
    return undefined;
  }
