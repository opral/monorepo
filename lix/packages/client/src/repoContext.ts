...
export async function repoContext(
  ...
  const rawFs =
    args.nodeishFs || (await import("@lix-js/fs")).createNodeishMemoryFs();
  ...
  if (url.startsWith("file:")) {
    ...
    if (origin.startsWith("git@githubClient.com:")) {
      url = origin.replace(
        "git@githubClient.com:",
        "https://githubClient.com/"
      );
    } else {
      url = origin;
    }
  ...
  const useLazyFS =
    experimentalFeatures?.lazyClone && rawFs?._createPlaceholder;
