...
export async function getMeta(ctx: RepoContext) {
  ...
  if (!gitUrl) {
    throw new Error(
      "Could not find repo url, only github supported for getMeta at the moment"
    );
  }
  ...
  if (!("error" in installResult)) {
    const found = installResult.installations.find(
      (i: any) => i.account.login === owner
    );
    ...
    if (found?.repository_selection === "all") {
      ...
      if (
        !("error" in repoResult) &&
        repoResult.repositories.find(
          (r: any) => r.full_name === `${owner}/${repoName}`
        )
      ) {
        isInstalled = true;
      }
