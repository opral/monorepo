...
export async function commit(
  ...
  if (include) {
    ...
    deletions.length &&
      (await Promise.all(deletions.map((del) => remove(ctx, state, del))));
  } else {
    // TODO: commit all
  }
...
export async function doCommit({
  ...
  async function createTree(
    ...
    if (!currentFolderStates) {
      throw new Error(
        "couldn't find folder " + currentFolder + " in file states"
      );
    }
  ...
  await walk({
    ...
    map: async function (filepath: string, [refState, stagingState]) {
      ...
      const stagingStateType = stagingState
        ? await stagingState.type()
        : undefined;

      // 'commit' used by TREE to represent submodules
      if (refStateType === "commit" || stagingStateType === "commit") {
        throw new Error(
          "Submodule found in " + filepath + " currently not supported"
        );
      }
      ...
      if (!stagingState && refState) {
        ...
        if (
          refStateType === "tree" ||
          (fs._isPlaceholder && fs._isPlaceholder(filepath))
        ) {
