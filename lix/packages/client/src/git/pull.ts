...
export async function pull(
  ...
  if (!ctx.gitUrl) {
    throw new Error(
      "Could not find repo url, only github supported for pull at the moment"
    );
  }

  const branchName =
    state.branchName ||
    (await isoGit.currentBranch({ fs: ctx.rawFs, dir: "/" })) ||
    "HEAD";
  ...
  if (ctx.experimentalFeatures.lazyClone) {
    ...
    ctx.debug &&
      console.info("experimental checkout after pull preload:", materialized);
  ...
  const mergeDriver = ({
    ...
    ctx.rawFs.writeFile(
      path + `.${branches[2]!.slice(0, 4)}.conflict`,
      contents[2] || ""
    );
