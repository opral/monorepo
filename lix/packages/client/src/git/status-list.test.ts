  ...
  async () => {
    async function doTest({ useLazy }: { useLazy: boolean }) {
      ...
      const repository: Awaited<ReturnType<typeof openRepository>> =
        await openRepository("https://github.com/inlang/ci-test-repo", {
          nodeishFs: fs,
          branch: "test-symlink",
          debug: false,
      ...
      if (useLazy) {
        ...
        expect(
          (
            await repository.statusList({
              includeStatus: ["materialized", "ignored"],
            })
      ...
      await repository.nodeishFs.writeFile(
      ...
      const statusResults = await repository.statusList({
        includeStatus: ["unmodified"],
      });
      expect(
        statusResults.filter(([, status]) => status !== "unmodified").length
      ).toBe(2);
      expect(
        statusResults.filter(([, status]) => status === "unmodified").length
      ).toBe(42);
      ...
      const statusPost3 = await repository.status(
      ...
      expect(
        await repository.statusList({ filepaths: ["postcss.config.js"] })
      ).toStrictEqual([
        [
          "postcss.config.js",
          "*undeleted",
      ...
      const fileContent3 = await repository.nodeishFs.readFile(
        "./vite.config.ts",
        {
          encoding: "utf-8",
        }
      ...
      const linkA = await repository.nodeishFs.readlink(
        "./test-symlink-not-existing-target"
      );
