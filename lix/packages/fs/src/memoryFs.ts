...
export function createNodeishMemoryFs(): NodeishFilesystem {
  ...
  async function stat(
    path: Parameters<NodeishFilesystem["stat"]>[0]
  ): Promise<NodeishStats> {
  ...
  return {
    ...
    readdir: async function (
      path: Parameters<NodeishFilesystem["readdir"]>[0]
    ) {
      path = normalizePath(path, {
        trailingSlash: "always",
        leadingSlash: "always",
    ...
    mkdir: async function mkdir(
      ...
      if (
        ...
        (parentDir && "palceholder" in parentDir)
      ) {
    ...
    watch: function (
      ...
      if (options?.persistent || options?.encoding) {
        throw new Error(
          "Some watch opptions not implemented, only 'recursive' allowed"
        );
      }
      ...
      let changeEvent: Promise<Error | undefined> = new Promise(
        (resolve, reject) => {
          handleNext = resolve;
          rejecteNext = reject;
        }
      ...
      const listener = ({ eventType, filename }: FileChangeInfo) => {
        ...
        if (event.filename === watchPath) {
          ...
          event.filename =
            event.filename.replace(`${watchPath}/`, "") || changeName;
          queue.push(event);
          setTimeout(() => handleNext(undefined), 0);
    ...
    symlink: async function (
      ...
      const rawTarget = target.startsWith("/")
        ? target
        : `${path}/../${target}`;
      ...
      const targetInode: Inode | undefined =
        state.fsMap.get(targetWithTrailing);
  ...
  function newStatEntry({
    ...
    const mtimeMs =
      ...
      (oldStats?.mtimeMs && Math.floor(oldStats?.mtimeMs / 1000))
        ? currentTime + 1000
        : currentTime;
