...
const runFsTestSuite = async (
  ...
  test("recursive mkdir", async () => {
    ...
    expect(await fs.readdir(`${tempDir}/home/user1/`)).toEqual([
      "documents",
      "downloads",
    ]);
  ...
  test("funny paths", async () => {
    ...
    expect(
      await fs.readdir(`${tempDir}/home/./../home/.//user1/documents`)
    ).toEqual([]);
  });
  ...
  test("file r/w", async () => {
    ...
    expect(
      await fs.readFile(`${tempDir}/home/user1/documents/file1`, {
        encoding: "utf-8",
      })
    ).toEqual(textInFirstFile);

    expect(
      await fs.readFile(`${tempDir}/file2`, { encoding: "utf-8" })
    ).toEqual(textInSecondFile);
  });
  ...
  test.skipIf(isNodeFs)("watch", async () => {
    ...
    const fileWatch = fs.watch(`${watchTempDir}/file`, {
    ...
    const folderWatch = fs.watch(`${watchTempDir}`, {
      signal: abortController.signal,
    });
    ...
    const fileEventFiles = new Set(
      fileWatchfsEvents.map((event) => event.filename)
    );
  ...
  test("r/w an empty file", async () => {
    ...
    expect(
      await fs.readFile(`${tempDir}/file3`, { encoding: "utf-8" })
    ).toEqual("");
  ...
  test("symlink", async () => {
    ...
    expect(
      await fs.readFile(`${tempDir}/file1.link`, { encoding: "utf-8" })
    ).toEqual(textInFirstFile);
    expect(
      await fs.readFile(`${tempDir}/file3.link`, { encoding: "utf-8" })
    ).toEqual("");
    expect(await fs.readdir(`${tempDir}/user1.link`)).toEqual([
      "documents",
      "downloads",
    ]);
  ...
  test("readlink", async () => {
    ...
    expect(await fs.readlink(`${tempDir}/file3.link`)).toEqual(
      `${tempDir}/file3`
    );

    expect(await fs.readlink(`${tempDir}/user1.link`)).toEqual(
      `${tempDir}/home/user1`
    );
  });
  ...
  test("stat/lstat", async () => {
    ...
    expect([
      stats.user1.isFile(),
      stats.user1.isDirectory(),
      stats.user1.isSymbolicLink(),
    ]).toEqual([false, false, true]);

    expect([
      stats.file1.isFile(),
      stats.file1.isDirectory(),
      stats.file1.isSymbolicLink(),
    ]).toEqual([false, false, true]);

    expect([
      stats.file2.isFile(),
      stats.file2.isDirectory(),
      stats.file2.isSymbolicLink(),
    ]).toEqual([true, false, false]);

    expect([
      stats.home.isFile(),
      stats.home.isDirectory(),
      stats.home.isSymbolicLink(),
  ...
  test.skipIf(isNodeFs)("placeholders", async () => {
    ...
    await expect(
      async () => await fs.readFile(`${placeholderPath}/test`)
    ).rejects.toThrow(/EPLACEHOLDER/);
  ...
  describe("throw errors", async () => {
    test("rm", async () => {
      await expect(
        async () => await fs.rm(`${tempDir}/home/dne/dne2`)
      ).rejects.toThrow(/ENOENT/);

      await expect(
        async () => await fs.rm(`${tempDir}/home/dne`)
      ).rejects.toThrow(/ENOENT/);

      await expect(
        async () => await fs.rm(`${tempDir}/home/user1`)
      ).rejects.toThrow(/EISDIR/);
    });

    test("mkdir", async () => {
      await expect(
        async () => await fs.mkdir(`${tempDir}/home/dne/dne2`)
      ).rejects.toThrow(/ENOENT/);
    });
    ...
    test("writeFile", async () => {
      await expect(
        async () =>
          await fs.readFile(`${tempDir}/home/dne/file`, { encoding: "utf-8" })
      ).rejects.toThrow(/ENOENT/);
    });
    ...
    test("readFile", async () => {
      await expect(
        async () =>
          await fs.readFile(`${tempDir}/home/dne`, { encoding: "utf-8" })
      ).rejects.toThrow(/ENOENT/);

      await expect(
        async () =>
          await fs.readFile(`${tempDir}/home/user1`, { encoding: "utf-8" })
      ).rejects.toThrow(/EISDIR/);
    });

    test("readdir", async () => {
      await expect(
        async () => await fs.readdir(`${tempDir}/home/dne`)
      ).rejects.toThrow(/ENOENT/);
  ...
  test("rm", async () => {
    await expect(
      async () => await fs.rm(`${tempDir}/home/user1/documents/`)
    ).rejects.toThrow(/EISDIR/);
    ...
    await expect(
      async () => await fs.readdir(`${tempDir}/home/user1`)
    ).rejects.toThrow(/ENOENT/);
