...
const PUBLIC_LIX_GITHUB_APP_CLIENT_ID = getEnvVar(
  "PUBLIC_LIX_GITHUB_APP_CLIENT_ID"
);
    ...
    if (
      installations.filter(
        (installation: any) =>
          ...
          user.id === installation.account?.id
      ).length === 0
    ) {
      // if app not installed, redirect via the install permissions url
      response.redirect(
        installUrl + "?state=" + encodeURIComponent(callbackUrl)
      );
    } else {
      response.redirect(callbackUrl);
    }
