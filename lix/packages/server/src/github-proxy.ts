...
router.all(
  ...
  async (request, response, next) => {
    try {
      const encryptedAccessToken = request.session?.encryptedAccessToken as
        | string
        | undefined;
      ...
      response.set(
        ...
        "x-github-api-version, user-agent, content-type"
      );
      response.set("Access-Control-Max-Age", "86400");

      if (
        targetUrl.endsWith("/user/emails") &&
        res.status === 401 &&
