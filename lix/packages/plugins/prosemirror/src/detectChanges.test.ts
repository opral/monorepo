...
test("it should detect multiple changes in a single document", async () => {
  ...
  const modifiedChange = detectedChanges.find(c => c.entity_id === "paragraph-2");
  const deletedChange = detectedChanges.find(c => c.entity_id === "paragraph-3");
  const addedChange = detectedChanges.find(c => c.entity_id === "paragraph-4");
  ...
  if (modifiedChange?.snapshot) {
    expect((modifiedChange.snapshot as any).content[0].text).toEqual("Second paragraph modified");
  }
  ...
  if (addedChange?.snapshot) {
    expect((addedChange.snapshot as any).content[0].text).toEqual("New fourth paragraph");
  }
});
...
test("it should detect changes in complex block structures like lists", async () => {
  ...
  const nestedListChange = detectedChanges.find(c => c.entity_id === "nested-list");
  ...
  if (item1Change?.snapshot) {
    expect((item1Change.snapshot as any).content[0].text).toEqual("First item modified");
  }
...
test("it should detect changes in tables", async () => {
  ...
  const cell11Change = detectedChanges.find(c => c.entity_id === "cell-1-1-p");
  ...
  if (cell11Change?.snapshot) {
    expect((cell11Change.snapshot as any).content[0].text).toEqual("Row 1, Cell 1 modified");
  }
...
test("it should detect changes in code blocks", async () => {
  ...
  if (change.snapshot) {
    ...
    expect((change.snapshot as any).content[0].text).toContain("function hello(): void");
  }
});
...
test("it should detect changes in blockquotes", async () => {
  ...
  const quoteParagraph1Change = detectedChanges.find(c => c.entity_id === "quote-p-1");
  const quoteParagraph2Change = detectedChanges.find(c => c.entity_id === "quote-p-2");
  ...
  if (quoteParagraph1Change?.snapshot) {
    expect((quoteParagraph1Change.snapshot as any).content[0].text).toEqual("Modified quote text");
  }
  
  // Verify paragraph 2 addition
  expect(quoteParagraph2Change?.snapshot).toBeDefined();
  if (quoteParagraph2Change?.snapshot) {
    expect((quoteParagraph2Change.snapshot as any).content[0].text).toEqual("Additional paragraph in quote");
  }
});
