...
test("it correctly applies changes to text with marks", async () => {
	...
	const { fileData: applied } = await applyChanges({
		lix,
		file: { id: "text-with-marks", path: "/text-with-marks.json", data: initial, metadata: null },
		changes,
	});
	...
	expect(paragraph.content[5].marks[0].attrs.href).toEqual("https://example.com");
});
...
test("it correctly applies changes to nested list structures", async () => {
	...
	const { fileData: applied } = await applyChanges({
		lix,
		file: { id: "nested-list", path: "/nested-list.json", data: initial, metadata: null },
		changes,
	});
	...
	expect(appliedDoc.type).toEqual("doc");
	expect(appliedDoc.content).toContainEqual(
		expectedNestedListDoc.content[0]
	);
});
...
test("it correctly applies changes to tables", async () => {
	...
	const { fileData: applied } = await applyChanges({
		lix,
		file: { id: "table-doc", path: "/table-doc.json", data: initial, metadata: null },
		changes,
	});
...
test("it correctly applies changes to blockquotes", async () => {
	...
	const { fileData: applied } = await applyChanges({
		lix,
		file: { id: "blockquote-doc", path: "/blockquote-doc.json", data: initial, metadata: null },
		changes,
	});
	...
	expect(appliedDoc.type).toEqual("doc");
	expect(appliedDoc.content).toContainEqual(
		expectedBlockquoteDoc.content[0]
	);
});
...
test("it correctly applies changes to mixed content", async () => {
	...
	const { fileData: applied } = await applyChanges({
		lix,
		file: { id: "mixed-content", path: "/mixed-content.json", data: initial, metadata: null },
		changes,
	});
	...
	const headingNode = appliedDoc.content.find((node: any) => node._id === "heading-1");
	...
	const codeNode = appliedDoc.content.find((node: any) => node._id === "code-1");
	expect(codeNode).toBeDefined();
	expect(codeNode?.attrs?.language).toEqual("typescript");
	expect(codeNode?.content?.[0]?.text).toEqual("console.log('hello typescript');");
});