...
const Editor: React.FC<EditorProps> = ({ onProposeChanges }) => {
	...
	const handleProposeChanges = async () => {
		// If onProposeChanges callback is provided, use it instead of creating a proposal
		if (onProposeChanges) {
			onProposeChanges();
			// Focus back to the editor after opening the proposal form
			view?.focus();
			//return;
		}

		// Legacy implementation for when onProposeChanges is not provided
		try {
