...
const Sidebar: React.FC<SidebarProps> = ({ initialTab = "checkpoints" }) => {
  const [activeTab, setActiveTab] = useState<SidebarTab>(initialTab);
  
  // Update the active tab whenever initialTab changes
  useEffect(() => {
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-auto">
        ...
        {activeTab === "proposals" && <ProposalList onCreateProposal={() => handleTabChange("create-proposal")} />}
        {activeTab === "create-proposal" && <ProposalForm onClose={handleCloseProposalCreate} />}
      </div>
    </div>
  );
};
...
export const showProposalCreate = (setTab: React.Dispatch<React.SetStateAction<SidebarTab>>) => {
