...
function App() {
  ...
  const handleDocChange = (newDoc: any) => {
    ...
    const processedDoc = ensureNodeStructure(JSON.parse(JSON.stringify(newDoc)));
  ...
  const detectDocChanges = async () => {
    ...
    try {
      ...
      if (manuallyDetectedChanges.length > 0) {
        ...
        console.log("Changes detected and added to the list:", manuallyDetectedChanges.length);
        return;
      }
      ...
      if (detectedChanges && detectedChanges.length > 0) {
        ...
        console.log("Changes detected and added to the list:", detectedChanges.length);
      } else {
      ...
      alert(`Error detecting changes: ${error.message}`);
    }
  };
  ...
  const handleRollbackChange = (change: any) => {
    ...
    try {
      ...
      if (change.snapshot) {
        console.log("Current document structure:", JSON.stringify(currentDoc, null, 2));
        ...
        if (change.entity_id.startsWith('paragraph-') || 
          ...
          if (!found) {
            console.log("Paragraph not found at top level, searching deeper...");
            
            // This recursive function tries to find and update a node by ID
            const updateNodeById = (node: any, id: string, snapshot: any): boolean => {
              ...
              if (node.content && Array.isArray(node.content)) {
                for (let i = 0; i < node.content.length; i++) {
                  ...
                  if (result && typeof result === 'object' && result !== true && result !== false) {
            ...
            const result = updateNodeById(updatedDoc, change.entity_id, change.snapshot);
          ...
          if (found) {
            ...
            setCurrentDoc(newDoc);
            alert(`Successfully rolled back change to paragraph ${change.entity_id}`);
            return;
          }
        }
        ...
        if (change.snapshot.type === 'paragraph' || 
      ...
      // For deletions (no snapshot), we need to restore from the previousDoc
        ...
        console.log("Handling a deletion - trying to restore node:", change.entity_id);
        ...
        if (deletedNode) {
          ...
          if (deletedNode.type !== "doc") {
            ...
            if (deletedNode.type === "paragraph" || 
              ...
              console.warn("Cannot restore this type of deleted node automatically:", deletedNode.type);
              alert(`Cannot automatically restore this type of node: ${deletedNode.type}`);
            }
          } else {
            // Replacing the whole document is dangerous
            console.warn("Cannot restore the root document node");
            alert("Cannot restore the document node itself - would replace entire document");
          }
        } else {
      ...
      alert(`Error rolling back change: ${error.message}`);
    }
  };
