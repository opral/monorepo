...
function assertParameter(name: string, value: any) {
	if (value === undefined) {
			...
			map: async function (filepath, [head, workdir, stage]) {
				...
				if (!head && !stage && workdir) {
					if (!shouldIgnore) {
						...
						if (ignored) {
							// "ignored"	file ignored by a .gitignore rule, will not be shown, todo: unless explicitly asked
							return null
						}
					}
				}
				...
				if (headType !== "blob" && workdirType === "blob" && stageType !== "blob") {
					...
					if (fs._isPlaceholder && fs._isPlaceholder(filepath)) {
						// 1, 2, 0 > check this meaning?
						// unmodified, todo: unless explicitly asked
						return null
					} else {
				...
				if (
					...
					return [filepath, "*undeletemodified"]
				}
