...
const runFsTestSuite = (name: string, tempDir: string, fs: Filesystem) =>
	describe(name, async () => {
		test("readdir", async () => {
		...
		describe("throw errors", async () => {
			test("rm", async () => {
				expect(async () => await fs.rm(`${tempDir}/home/dne/dne2`)).rejects.toThrow(/ENOENT/)

				expect(async () => await fs.rm(`${tempDir}/home/dne`)).rejects.toThrow(/ENOENT/)

				expect(async () => await fs.rm(`${tempDir}/home/user1`)).rejects.toThrow(/EISDIR/)
			})

			test("mkdir", async () => {
				expect(async () => await fs.mkdir(`${tempDir}/home/dne/dne2`)).rejects.toThrow(/ENOENT/)
			})

			test("writeFile", async () => {
				expect(async () => await fs.readFile(`${tempDir}/home/dne/file`, "utf8")).rejects.toThrow(
					/ENOENT/,
				)
			})

			test("readFile", async () => {
				expect(async () => await fs.readFile(`${tempDir}/home/dne`, "utf8")).rejects.toThrow(
					/ENOENT/,
				)

				expect(async () => await fs.readFile(`${tempDir}/home/user1`, "utf8")).rejects.toThrow(
					/EISDIR/,
				)
			})

			test("readdir", async () => {
				expect(async () => await fs.readdir(`${tempDir}/home/dne`)).rejects.toThrow(/ENOENT/)
		...
		test("fromJson", async () => {
			const fsJson = await fs.toJson({ dir: tempDir })
			await fs.rm(tempDir, { recursive: true }).catch((err) => {
