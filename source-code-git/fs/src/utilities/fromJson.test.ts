...
it("should be able to import files from a JSON", async () => {
...
it("should be able to import files from base64 JSON", async () => {
	const fs = createMemoryFs()
	await fromJson({
		fs,
		resolveFrom: "/",
...
it("should be able to make a base64 roundtrip", async () => {
	const fs = createMemoryFs()
	await fromJson({
		fs,
		resolveFrom: "/",
	...
	const json = await toJson({ fs, matchers: ["**/*"], resolveFrom: "/", encodeOutput: "base64" })
	const fs2 = createMemoryFs()
	await fromJson({ fs: fs2, resolveFrom: "/", decodeInput: "base64", json })
	expect(await fs.readFile("/file1.txt", { encoding: "utf-8" })).toEqual(
		await fs2.readFile("/file1.txt", { encoding: "utf-8" }),
	)
	expect(await fs.readFile("/file2.js", { encoding: "utf-8" })).toEqual(
