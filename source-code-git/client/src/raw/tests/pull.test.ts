...
describe("pull", () => {
	it("pull", async () => {
		...
		expect(logs.map(({ commit }) => commit.message)).toEqual(["Initial commit\n"])
		await pull({
			fs,
			http,
			gitdir,
	...
	it("pull fast-forward only", async () => {
		...
		expect(logs.map(({ commit }) => commit.message)).toEqual(["Added z.txt\n", "Initial commit\n"])
		let err = null
		try {
			await pull({
				fs,
	...
	it("pull no fast-forward", async () => {
		...
		logs = await log({ fs, gitdir, dir, ref: "refs/heads/master" })
		const formattedLogs = logs.map(({ commit }) => `${commit.message} (${commit.parent.join(" ")})`)
