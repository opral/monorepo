...
import { Errors, merge, add, resolveRef, log } from "isomorphic-git"
...
describe("merge", () => {
	...
	it("merge 'delete-first-half' and 'delete-second-half' (dryRun)", async () => {
		...
		expect(
			await fs.exists(`${gitdir}/objects/${report.oid.slice(0, 2)}/${report.oid.slice(2)}`),
		).toBe(false)
	})
	...
	it("merge two branches that modified the same file, no conflict resolver (should conflict)'", async () => {
		...
		expect(await fs.read(outFile, "utf-8")).toBe(await fs.read(testFile, "utf-8"))
		expect(error).not.toBeNull()
