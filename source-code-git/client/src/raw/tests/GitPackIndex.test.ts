...
import { GitPackIndex, GitObject, shasum } from "isomorphic-git/internal-apis.js"

describe("GitPackIndex", () => {
	it("from .idx", async () => {
		const { fs, gitdir } = await makeFixture("test-GitPackIndex")
		const idx = await fs.read(
			path.join(gitdir, "objects/pack/pack-1a1e70d2f116e8cb0cb42d26019e5c7d0eb01888.idx"),
		)
		const p = await GitPackIndex.fromIdx({ idx })
		expect(await shasum(Buffer.from(JSON.stringify(p.hashes)))).toMatchInlineSnapshot(
			'"fd2404a29d1e5dc72066541366d5f75bc9d51c9b"',
		)
	...
	it("from .pack", async () => {
		...
		const pack = await fs.read(
			path.join(gitdir, "objects/pack/pack-1a1e70d2f116e8cb0cb42d26019e5c7d0eb01888.pack"),
		)
