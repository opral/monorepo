...
describe("statusMatrix", () => {
	...
	describe("ignored:true works supports multiple filepaths", () => {
		...
		beforeAll(async () => {
			...
			await fs.write(path.join(dir, nonIgnoredFolder, "notIgnored.txt"), "notIgnored")
	...
	it("statusMatrix with removed folder and created file with same name", async () => {
		// Setup
		const { fs, dir, gitdir } = await makeFixture("test-statusMatrix-tree-blob-collision")
		// Test
	...
	it("statusMatrix with removed file and created folder with same name", async () => {
		// Setup
		const { fs, dir, gitdir } = await makeFixture("test-statusMatrix-blob-tree-collision")
		// Test
