...
import { init, add, listFiles, readBlob, walk, STAGE } from "isomorphic-git"
...
const writeGitIgnore = async (fs, dir) =>
	fs.write(dir + "/.gitignore", ["*-pattern.js", "i.txt", "js_modules", ".DS_Store"].join("\n"))

// NOTE: we cannot actually commit a real symlink in fixtures because it relies on core.symlinks being enabled
const writeSymlink = async (fs, dir) => fs._symlink("c/e.txt", dir + "/e-link.txt")
