...
describe("branch", () => {
	...
	it("branch with start point", async () => {
		...
		// Test
		let files = await fs.readdir(path.join(gitdir, "refs", "heads"))
		expect(files).toEqual(["main", "start-point"])
		await branch({ fs, dir, gitdir, ref: "test-branch", object: "start-point" })
		files = await fs.readdir(path.join(gitdir, "refs", "heads"))
		expect(files).toEqual(["main", "start-point", "test-branch"])
		expect(await currentBranch({ fs, dir, gitdir })).toEqual("main")
		expect(await fs.read(path.join(gitdir, "refs", "heads", "test-branch"), "utf8")).toEqual(
			await fs.read(path.join(gitdir, "refs", "heads", "start-point"), "utf8"),
		)
		expect(await listFiles({ fs, dir, gitdir, ref: "HEAD" })).toEqual(["new-file.txt"])
	...
	it("branch force", async () => {
		...
		await branch({ fs, dir, gitdir, ref: "test-branch" })
		expect(await currentBranch({ fs, dir, gitdir })).toEqual("master")
		expect(await fs.exists(path.join(gitdir, "refs/heads/test-branch"))).toBeTruthy()
		try {
	...
	it("branch with start point force", async () => {
		...
		expect(error).toBeNull()
		expect(await listFiles({ fs, dir, gitdir, ref: "test-branch" })).toEqual(["new-file.txt"])
	})
	...
	it("empty repo", async () => {
		...
		expect(error).toBeNull()
		const file = await fs.read(path.join(gitdir, "HEAD"), "utf8")
	...
	it("create branch with same name as a remote", async () => {
		...
		expect(error).toBeNull()
		expect(await fs.exists(path.join(gitdir, "refs/heads/origin"))).toBeTruthy()
	})
	...
	it('create branch named "HEAD"', async () => {
		...
		expect(error).toBeNull()
		expect(await fs.exists(path.join(gitdir, "refs/heads/HEAD"))).toBeTruthy()
	})
})
