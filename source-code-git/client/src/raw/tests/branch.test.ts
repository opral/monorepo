...
describe('branch', () => {
  ...
  it('branch with start point', async () => {
    ...
    expect(
      await fs.read(
        path.join(gitdir, 'refs', 'heads', 'test-branch'),
        'utf8'
      )
    ).toEqual(
      await fs.read(
        path.join(gitdir, 'refs', 'heads', 'start-point'),
        'utf8'
      )
    )
  ...
  it('branch force', async () => {
    ...
    expect(await currentBranch({ fs, dir, gitdir })).toEqual('master')
    expect(
      await fs.exists(path.join(gitdir, 'refs/heads/test-branch'))
    ).toBeTruthy()
    try {
  ...
  it('create branch with same name as a remote', async () => {
    ...
    expect(error).toBeNull()
    expect(
      await fs.exists(path.join(gitdir, 'refs/heads/origin'))
    ).toBeTruthy()
  })
  ...
  it('create branch named "HEAD"', async () => {
    ...
    expect(error).toBeNull()
    expect(
      await fs.exists(path.join(gitdir, 'refs/heads/HEAD'))
    ).toBeTruthy()
  })
})
