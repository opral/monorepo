...
import { Errors, checkout, clone, currentBranch, resolveRef, getConfig } from "isomorphic-git"
...
describe("clone", () => {
	...
	it.skip("clone with noTags", async () => {
		...
		expect(await resolveRef({ fs, gitdir, ref: "refs/remotes/origin/test-branch" })).toBe(
			"e10ebb90d03eaacca84de1af0a59b444232da99e",
		)
		expect(await resolveRef({ fs, gitdir, ref: "refs/heads/test-branch" })).toBe(
			"e10ebb90d03eaacca84de1af0a59b444232da99e",
		)
		let err = null
		try {
	...
	it("clone with noCheckout", async () => {
		...
		expect(await fs.exists(`${dir}`)).toBe(true)
		expect(await fs.exists(`${gitdir}/objects`)).toBe(true)
		expect(await fs.exists(`${gitdir}/refs/remotes/origin/test-branch`)).toBe(true)
	...
	it("clone from git-http-mock-server", async () => {
		...
		expect(await fs.exists(`${gitdir}/objects`)).toBe(true, `'gitdir/objects' exists`)
	...
	if (typeof process === "object" && (process.versions || {}).node) {
		it.skip("should allow agent to be used with built-in http plugin for Node.js", async () => {
			const { fs, dir, gitdir } = await makeFixture("isomorphic-git")
			const connectionLog = []
