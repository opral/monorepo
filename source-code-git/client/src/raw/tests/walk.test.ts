...
// @ts-nocheck
import { walk, WORKDIR, TREE, STAGE } from "isomorphic-git"
		...
		const matrix = await walk({
			...
			map: async (filepath, [workdir, tree, stage]) => [
				...
				workdir && {
					...
					content:
						(await workdir.content()) && Buffer.from(await workdir.content()).toString("utf8"),
				...
				tree && {
					...
					oid: await tree.oid(),
					content: (await tree.content()) && Buffer.from(await tree.content()).toString("utf8"),
				...
				stage && {
					...
					oid: await stage.oid(),
					content: (await stage.content()) && Buffer.from(await stage.content()).toString("utf8"),
