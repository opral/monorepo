...
describe("transformMessageId function", () => {
  ...
  it("should remove special characters", () => {
    ...
    expect(transformMessageId("hello-^%#@$world!@#$%^&*()")).toEqual(
      "helloworld",
    );
  });
  ...
  it("should replace multiple spaces with underscores", () => {
    ...
    expect(transformMessageId("hello  world   message")).toEqual(
      "hello_world_message",
    );
  });
  ...
  it("should prepend an underscore if the transformed ID starts with a number", () => {
    ...
    expect(transformMessageId("12345.5 HELLO_WORLD")).toEqual(
