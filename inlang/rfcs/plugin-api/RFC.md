...
- Consider error handling and error messages. We want to make sure that the user gets meaningful error messages if something goes wrong. Numerous developers ran into https://github.com/inlang/monorepo/issues/267, wasting hours on debugging.
...
- Plugin/app-specific configuration is leaking into the core config, see https://github.com/inlang/monorepo/blob/main/inlang/source-code/versioned-interfaces/project-config/src/interface.ts#L70-L122. This is bad. The result will be a massive config schema that becomes unmaintainable.
...
1. Have a utility function called `createPlugin` [similar to `createLintRule` in the lint rule system](https://github.com/inlang/monorepo/blob/main/inlang/source-code/versioned-interfaces/message-lint-rule/src/interface.ts#L21-L49).
