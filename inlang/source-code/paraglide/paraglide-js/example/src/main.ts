...
test("should log the messages in the correct locale", async () => {
	...
	expect(m.blue_horse_shoe({ username: "Anna", placename: "New York City" })).toBe(
		"Hallo Anna! Du bist in New York City."
	)
})

test("matching should work", async () => {
	expect(m.jojo_mountain_day({ platform: "android", username: "Samuel", userGender: "male" })).toBe(
		"Samuel has to download the app on his phone from the Google Play Store."
	)

	expect(
		m.jojo_mountain_day({ platform: "someting", userGender: "other", username: "unknown" })
	).toBe("The person has to download the app.")
})

test("defining a locale as option should work ", async () => {
	setLocale("en")
	expect(m.blue_horse_shoe({ username: "Samuel", placename: "Berlin" }, { locale: "de" })).toBe(
