...
describe("e2e", async () => {
	...
	test("should return the correct message if a languageTag is set in the message options", async () => {
		...
		expect(m.sad_penguin_bundle()).toBe("Eine einfache Nachricht.")
		expect(m.sad_penguin_bundle(undefined, { languageTag: "en" })).toBe("A simple message.")
	})
			...
			bundles: [
				createBundleNested({
					...
					messages: [
						...
						{
							locale: "en-US",
							variants: [{ pattern: [{ type: "text", value: "A simple message for Americans." }] }],
						},
					],
				}),
				createBundleNested({
					...
					messages: [
						{
							locale: "en",
							variants: [{ pattern: [{ type: "text", value: "Fallback message." }] }],
						},
					],
				}),
			],
...
const mockBundles: BundleNested[] = [
	createBundleNested({
		...
		messages: [
			...
			{
				locale: "de",
				variants: [{ pattern: [{ type: "text", value: "Eine einfache Nachricht." }] }],
			},
		],
	}),
	{
