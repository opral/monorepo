...
describe("preferredLanguages", () => {
	it("should return the acceptable languages", () => {
		expect(negotiateLanguagePreferences("de-CH", ["de", "de-CH"])).toEqual(["de-CH", "de"])
	})
	...
	it("should sort the matches by quality", () => {
		...
		expect(negotiateLanguagePreferences(headerValue, ["fr-CH", "fr", "en", "de"])).toEqual([
			"fr-CH",
			"en",
			"de",
			"fr",
	...
	it("should return the available languages if the accept header is missing", () => {
		expect(negotiateLanguagePreferences(undefined, ["en-US", "en-GB", "de"])).toEqual([
			"en-US",
			"en-GB",
			"de",
		])
