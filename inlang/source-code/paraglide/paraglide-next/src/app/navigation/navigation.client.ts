...
export const Navigation = <T extends string>({
  ...
  const { redirect, permanentRedirect } = createRedirects(
    languageTag,
    strategy,
  );
 ...
 */
function createUsePathname<T extends string>(
  strategy: RoutingStrategy<T>,
): LocalizedUsePathname {
  return (): `/${string}` => {
    const encodedLocalisedPathname = NextNavigation.usePathname();
    const localisedPathname = decodeURI(
      encodedLocalisedPathname,
    ) as `/${string}`;
...
> & {
  push: (
    ...
    options?:
      | (Parameters<AppRouterInstance["push"]>[1] & { locale?: T })
      | undefined,
  ...
  replace: (
    ...
    options?:
      | (Parameters<AppRouterInstance["replace"]>[1] & { locale?: T })
      | undefined,
  ...
  prefetch: (
    ...
    options?:
      | (Parameters<AppRouterInstance["prefetch"]>[1] & { locale?: T })
      | undefined,
