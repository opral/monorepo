  ...
  it.each(availableLanguageTags)(
    ...
    (languageTag) => {
      ...
      const request = new NextRequest(
        "https://example.com/" + languageTag + "/some-page",
      );
      const response = middleware(request);
...
describe("Middleware with Domain Strategy", () => {
  ...
  it.each(availableLanguageTags)(
    "Detects the language from the domain",
    (languageTag) => {
...
function expectHeaderValue(
  ...
  expect(response.headers.get("x-middleware-override-headers")).includes(
    headerName,
  );
  expect(response.headers.get("x-middleware-request-" + headerName)).toBe(
    headerValue,
  );
}

function expectCookieValue(
  response: NextResponse,
  cookieName: string,
  cookieValue: string,
) {
