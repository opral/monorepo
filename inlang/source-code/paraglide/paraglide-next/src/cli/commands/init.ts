...
> = async (ctx) => {
	...
	try {
		fileContent = await ctx.repo.nodeishFs.readFile(ctx.nextProject.configFile.path, {
			encoding: "utf-8",
		})
	} catch (e) {
		ctx.logger.error("Failed to read next config file at " + ctx.nextProject.configFile.path)
		process.exit(1)
	}
...
`
	...
	...${configIdentifier}
		...
		await ctx.repo.nodeishFs.writeFile(ctx.nextProject.configFile.path, fileContent)
	}
