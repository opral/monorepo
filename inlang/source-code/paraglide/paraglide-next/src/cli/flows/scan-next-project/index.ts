...
> = async (ctx) => {
  ...
  if (!packageJsonPath) {
    ctx.logger.error(
      `Could not find package.json. Rerun this command inside a NextJS project.`,
    );
    process.exit(1);
  }
  ...
  if (!configFile) {
    ctx.logger.error(
      `Could not find Next Config File. Rerun this command inside a NextJS project.`,
    );
    process.exit(1);
  }
  ...
  const srcRoot = (await folderExists(
    ctx.repo.nodeishFs,
