...
async function promptDomainForLanguage(
  languageTag: string,
  logger: Logger,
): Promise<URL> {
  const response = await prompt(
    `Which domain do you want to use for "${languageTag}"?`,
    {
      type: "text",
  ...
  logger.warn(
    "Please enter a valid domain, including the protocol. Example: https://example.com",
  );
