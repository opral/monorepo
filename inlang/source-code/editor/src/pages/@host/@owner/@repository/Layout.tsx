...
export function Layout(props: { children: JSXElement }) {
  ...
  const removeFilter = (filterName: string) => {
    setSelectedFilters(
      selectedFilters().filter((filter: Filter) => filter.name !== filterName),
    );
  };
  ...
  const [runSettingsCloseAnimation, setRunSettingsCloseAnimation] =
    createSignal(false);
  const handleClose = () => {
    ...
    <EditorLayout>
          ...
          >
            <div class="relative w-full max-w-7xl mx-auto px-4 lg:pt-12">
              ...
              <div class="max-w-screen-sm mx-auto">
                ...
                <inlang-settings
                  prop:settings={
                    project()!.settings() as ReturnType<
                      InlangProject["settings"]
                    >
                  }
                  ...
                  on:set-settings={(event: CustomEvent) => {
                    ...
                    if (_project) {
                      ...
                      throw new Error(
                        "Settings can not be set, because project is not defined",
                      );
                    }
                  }}
                />
      ...
      >
        <p class="text-sm pb-4 -mt-4 pr-8">
          Your fork is out of sync with the upstream repository. Please resolve
          the conflicts before applying your changes.
        </p>
        <img
      ...
      >
        ...
        <sl-menu class="w-48 min-w-fit">
          <Show
            when={branchList()}
            fallback={
              <sl-menu-item prop:disabled={true}>Loading...</sl-menu-item>
            }
          >
            <For each={branchList()}>
              {(branch) => (
...
function LanguageFilter(props: {
  ...
  onMount(() => {
    if (
      filteredLanguageTags().length === 0 ||
      filteredLanguageTags() === undefined
    ) {
      setFilteredLanguageTags(languageTags());
    }
  });
...
function LintFilter(props: { clearFunction: any }) {
  const { project, filteredMessageLintRules, setFilteredMessageLintRules } =
    useEditorState();
  return (
    <sl-select
        ...
        >
          ...
          <button
            ...
            onClick={() => {
              setFilteredIds(
                setFilteredIds(
                  project()?.query.messages.includedMessageIds() ?? [],
                ),
              );
              props.clearFunction;
            }}
