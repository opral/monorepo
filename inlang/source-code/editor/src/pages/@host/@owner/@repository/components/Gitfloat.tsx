...
import {
  TourHintWrapper,
  type TourStepId,
} from "./Notification/TourHintWrapper.jsx";
...
export const Gitfloat = () => {
  ...
  const gitState: () =>
    ...
    const isFork = () =>
      repoInfo && "isFork" in repoInfo ? repoInfo.isFork : false;
  ...
  const data: GitFloatArray = {
    ...
    hasChanges: {
      ...
      icon: IconPush,
      onClick: () =>
        triggerPushChanges("chore: update translations with Fink 🐦"),
    },
    pullrequest: {
      text: "",
      buttontext: refLink().includes("ninja-")
        ? "View pull request"
        : "Open pull request",
  ...
  createEffect(() => {
    if (
      ...
      setTimeout(
        () => gitfloat?.classList.remove("animate-slideInFromBehind", "-z-10"),
        400,
      );
    }
  });
  ...
  createEffect(() => {
    if (!isMerging()) {
      ...
      setTimeout(
        () => gitfloat?.classList.remove("animate-slideOutFromBehind", "-z-10"),
        400,
      );
  ...
  ): { username: string; avatarUrl?: string } | false =>
    user?.isLoggedIn ? user : false;
    ...
    <>
      <div class="gitfloat z-30 sticky mx-auto bottom-8 w-[300px] my-16">
        <TourHintWrapper
          ...
          offset={{ x: 0, y: 60 }}
          isVisible={
            (tourStep() === "github-login" ||
              tourStep() === "fork-repository") &&
            project() !== undefined &&
            gitState() !== "loading" &&
          ...
          <div class="w-full flex justify-start items-center rounded-lg bg-inverted-surface shadow-xl z-20">
            ...
            >
              ...
              <sl-button
                ...
                prop:href={
                  data[gitState()].href === "pullrequest"
                    ? pullrequestUrl()
                    : undefined
                }
                prop:target="_blank"
                ...
                prop:disabled={
                  localChanges() === 0 && gitState() === "hasChanges"
                }
                class={
                  "on-inverted " +
                  (gitState() === "pullrequest" ? "grow " : "w-[76.8px]")
                }
              >
      ...
      >
        ...
        <p>
          To push changes, you must use a fork of this repository and submit
          changes with a pull request. If you already have one, this will be
          opened, otherwise a new fork will be created for you.
        </p>
        <sl-button
          slot="footer"
