...
import {
  type Notification,
  NotificationHint,
} from "./Notification/NotificationHint.jsx";
...
export function PatternEditor(props: {
  ...
  const [variableReferences, setVariableReferences] = createSignal<
    VariableReference[]
  >([]);
  ...
  const variant = () =>
    getVariant(props.message, { where: { languageTag: props.languageTag } });
  ...
  createEffect(
    ...
    on(
      ...
      debounce(500, () => {
        if (
          JSON.stringify(currentJSON().content) !==
      ...
      if (
        ...
        notifications.push({
          ...
          notificationDescription:
            typeof report.body === "object" ? report.body.en : report.body,
    ...
    if (
      ...
      timer = setTimeout(() => {
        showToast({
          variant: "info",
          title:
            "Inlang saves automatically, but make sure you push your changes.",
        });
      }, 500);
    }
  };
            ...
            <sl-button
              ...
              onClick={() => {
                editor().commands.setContent(
                  setTipTapMessage(referencePattern()!),
                );
        ...
        <Show when={showMachineLearningWarningDialog()}>
          ...
          >
            <ol class="">
              ...
              <br />
              <li>
                2. Machine translations do not exclude placeholders like "My
                name is{" "}
                <code class="bg-surface-1 py-0.5 px-1 rounded">{"{name}"}</code>
                {'" '}
                yet. Make sure that placeholders between the reference message
                and translations match. For more information read{" "}
                <Link
