...
type EditorStateSchema = {
  ...
  mergeUpstream: () => Promise<
    Awaited<ReturnType<Repository["mergeUpstream"]>>
  >;
  ...
  githubRepositoryInformation: Resource<
    Awaited<ReturnType<Repository["getMeta"]>> | undefined
  >;
  /**
...
export function EditorStateProvider(props: { children: JSXElement }) {
  ...
  const [filteredIds, setFilteredIds] = createSignal<string[]>(
    params.getAll("id") as string[],
  );
  createEffect(() => {
  ...
  const [filteredLanguageTags, setFilteredLanguageTags] = createSignal<
    LanguageTag[]
  ...
  const [refLink, setRefLink] = createSignal<string>(
    (params.get("ref") || "") as string,
  );
  createEffect(() => {
  ...
  async function pushChanges(args: {
    ...
    try {
      ...
      if (push?.error) {
        return {
          error: new PushException("Failed to push.", { cause: push.error }),
        };
      }
  ...
  async function mergeUpstream(): Promise<
    Awaited<ReturnType<Repository["mergeUpstream"]>>
  > {
    const loadedRepo = repo();
    if (!loadedRepo) {
  ...
  const [projectList] = createResource(
    ...
    async (args) => {
      ...
      if (
        ...
        projectList.some(
          (project: any) => project.projectPath === searchParams().project,
        )
      ) {
        ...
        projectList.some(
          (project: any) => project.projectPath === "/project.inlang",
        )
      ) {
  ...
  const [project, { refetch: refetchProject, mutate: setProject }] =
    createResource(
      () => {
      ...
      async ({ newRepo, activeProject }) => {
        ...
        if (newRepo) {
          ...
          telemetryBrowser.capture("EDITOR cloned repository", {
            userPermission: userIsCollaborator()
              ? "iscollaborator"
              : "isNotCollaborator",
          });
          return project;
  ...
  const [shouldRecommendNinja, { refetch: refetchShouldRecommendNinja }] =
    createResource(
      () => {
  ...
  async function addNinja(
    ...
    try {
      ...
      console.error(
        "Failed to add the Ninja Github Action. Please open an issue",
      );
    }
  }
    ...
    if (localStorage?.user === undefined || userIsCollaborator.loading) {
      ...
      if (project()?.installed.messageLintRules().length === 0) {
        ...
        setTimeout(() => {
          ...
          element !== null &&
          !filteredMessageLintRules().includes(
  ...
  const [githubRepositoryInformation, { refetch: refetchRepoInfo }] =
    createResource(
      () => {
        const loadedRepo = repo();
        if (
  ...
  const [forkStatus, { refetch: refetchForkStatus, mutate: mutateForkStatus }] =
    createResource(
      () => {
  ...
  const [previousLoginStatus, setPreviousLoginStatus] = createSignal(
