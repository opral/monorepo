...
test("roundtrip readJSON/writeJSON", async () => {
  ...
  const afterRoundtrip = await nodeishFs.readFile(filePath, {
    encoding: "utf-8",
  });
...
test("openStore does minimal CRUD on messageBundles", async () => {
  ...
  const firstMessageBundle = await store.messageBundles.get({
    id: "first_message",
  });
  expect(firstMessageBundle).toStrictEqual(mockMessages[0]);

  const modifedMessageBundle = structuredClone(
    firstMessageBundle,
  ) as MessageBundle;
  const newMessage = createMessage({
    locale: "de",
    text: "Wenn dies schiefläuft, bin ich sauer",
  });
  modifedMessageBundle.messages.push(newMessage);
  await store.messageBundles.set({ data: modifedMessageBundle });

  const setMessageBundle = await store.messageBundles.get({
    id: "first_message",
  });
