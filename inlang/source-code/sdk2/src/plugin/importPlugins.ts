...
export async function importPlugins(args: {
	...
	for (const uri of args.settings.modules) {
		...
		try {
			...
			const preprocessed = (await args.preprocessPluginBeforeImport?.(moduleAsText)) ?? moduleAsText
			const moduleWithMimeType = "data:application/javascript," + encodeURIComponent(preprocessed)
			const { default: plugin } = await import(/* @vite-ignore */ moduleWithMimeType)
			plugins.push(plugin)
		} catch (e) {
...
async function fetchModuleWithCache(uri: string): Promise<string> {
	...
	if (!response.ok) {
		throw new Error(`Failed to fetch the plugin. Is the link ${uri} valid? ${response.statusText}`)
	}
