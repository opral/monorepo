// NOTE: This a copy of the chokidar sources version 4 - two reasonse we inlined this file into the repo:
// 1. chokidar has a subdependency to readdirp that uses fs - sub dependency mocking / inlining is not supported in vite?
// 2. the version 4 of chokidar - that uses import instead of require is not yet in npm but require is not supported for mocking in vite see: https://github.com/vitest-dev/vitest/discussions/3134
...
const normalizePathToUnix = (path: Path) =>
...
export class WatchHelper {
	...
	constructor(path: string, follow: boolean, fsw: any) {
		...
		/** @type {object|boolean} */
		this.dirParts = [];
		for (const parts of this.dirParts) {
...
export class FSWatcher extends EventEmitter {
	...
	close() {
		...
		const closers: any[] = [];
		for (const closerList of this._closers)
			for (const closer of closerList) {
				const promise = closer[1]();
		...
		this._readyCount = 0;
		this._readyEmitted = false;
		for (const dirent of this._watched) dirent[1].dispose();
	...
	getWatched() {
		...
		for (const [dir, entry] of this._watched.entries()) {
	...
	_remove(directory: string, item: string, isDirectory?: boolean): void {
		...
		for (const nested of nestedDirectoryChildren) this._remove(path, nested);
